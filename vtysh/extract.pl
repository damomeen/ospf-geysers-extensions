...
##### G(2)MPLS  additions
$ignore{'"scngws"'       } = "ignore";
$ignore{'"lrm"'          } = "ignore";
$ignore{'"tnrc"'         } = "ignore";
$ignore{'"g2rsvpte"'     } = "ignore";
$ignore{'"gunigw"'       } = "ignore";
$ignore{'"g2pcera"'      } = "ignore";
$ignore{'"ncc"'          } = "ignore";

foreach (@ARGV) {
    $file = $_;

    open (FH, "cpp -DHAVE_CONFIG_H -DVTYSH_EXTRACT_PL -DHAVE_IPV6 -I.. -I./ -I./.. -I../idl -I../lib -I../isisd/topology @LIBG2MPLS_INCLUDES@  $file |");
    local $/; undef $/;
    $line = <FH>;
    close (FH);
	...
	$cmd =~ s/\s+$//g;
	   ...
	   if ($file =~ /^.*\.c$/) {
	       ...
	       ($protocol) = ($file =~ /^.*\/([a-z0-9]+)\/[a-zA-Z0-9_\-]+\.cxx$/);
	   }
