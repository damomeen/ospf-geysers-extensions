...
static void
config_add_line (struct zlist *config, const char *line)
{
  listnode_add (config, XSTRDUP (MTYPE_VTYSH_CONFIG_LINE, line));
}

static void
config_add_line_uniq (struct zlist *config, const char *line)
{
  struct zlistnode *node, *nnode;
  char *pnt;
