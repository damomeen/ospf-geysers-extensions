...
vclient_close (struct vtysh_client *vclient)
{
	if (vclient->fd >= 0) {
		fprintf(stderr,
			"Warning: closing connection to %s "
...
vtysh_client_config (struct vtysh_client *vclient, char *line)
{
	...
	/*
	 * Allow enough room for buffer to read more
	 * than a few pages from socket.
	 */
	...
	while (1) {
		...
		/*
		 * See if a line exists in buffer,
		 * if so parse and consume it, and
		 * reset read position.
		 */
		if ((eoln = strrchr(buf, '\n')) == NULL)
			continue;
...
vtysh_execute_func (const char *line, int pager)
{
	 ...
	 * in node tree.
	 * Changing vty->node is enough to try it just out without actual
	 * walkup in the vtysh.
	 */
	...
	/* If command succeeded in any other node than current (tried > 0)
	 * we have to move into node in the vtysh where it succeeded.
	 */
	if (ret == CMD_SUCCESS ||
	    ret == CMD_SUCCESS_DAEMON ||
	    ret == CMD_WARNING) {
		...
		/* If command didn't succeed in any node, continue with
		 * return value from first try.
		 */
		ret = saved_ret;
	}
	...
	switch (ret) {
		...
		case CMD_SUCCESS_DAEMON: {
			/* FIXME: Don't open pager for exit
			 * commands. popen() causes problems
			 * if exited from vtysh at all.
			 * This hack shouldn't cause any
			 * problem but is really ugly.
			 */
			if (pager &&
			    vtysh_pager_name &&
			...
			if (! strcmp(cmd->sstring,"configure terminal")) {
	...
	while (fgets (vty->buf, VTY_BUFSIZ, fp)) {
		...
		if (ret != CMD_SUCCESS &&
			...
			if (vty->node == KEYCHAIN_KEY_NODE) {
				...
				ret = cmd_execute_command_strict (vline,
								  vty,
								  &cmd);
...
DEFUNSH_DEPRECATED (VTYSH_ALL,
		    ...
		    "log trap "LOG_LEVELS,
		    "Logging control\n"
		    "(Deprecated) Set logging level and "
		    "default for all destinations\n"
		    LOG_LEVEL_DESC)
...
#ifdef DEBUG
	fprintf(stderr, "vtysh_connect(%s): connected! %s\n", vclient->path);
#endif /* DEBUG */
	...
	for (i = 0; i < VTYSH_INDEX_MAX; i++) {
		if (!daemon_name ||
			...
			if (vtysh_connect(&vtysh_client[i]) == 0)
				rc++;
			/* We need direct access to ripd in
			 * vtysh_exit_ripd_only.
			 */
