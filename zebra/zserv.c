...
zsend_interface_add (struct zserv *client, struct interface *ifp)
{
...
#ifdef GMPLS
  /*Send TE params along with the interface*/
  zlog_debug("zsend_interface_add: interface adjacency %d, ospf instance %d",
	     ifp->adj_type, ifp->ospf_instance);
  stream_putw (s, ifp->adj_type);
  stream_putw (s, ifp->ospf_instance);
  stream_putl(s, ifp->te_local_id);
  stream_putl(s, ifp->te_remote_id);
  stream_putl(s, ifp->te_remote_node_id);
  stream_putl(s, ifp->te_metric);
  stream_putl(s, ifp->te_link_color);
  stream_putl(s, ifp->te_swcap);
  stream_putl(s, ifp->te_swcap_options);
  stream_putl(s, ifp->te_enctype);
  stream_putl(s, ifp->te_max_bw);
  stream_putl(s, ifp->te_max_res_bw);

  for (i=0; i< MAX_BW_PRIORITIES; i++)
    stream_putl(s, ifp->te_avail_bw_per_prio[i]);
  for (i=0; i< MAX_BW_PRIORITIES; i++)
    stream_putl(s, ifp->te_max_LSP_bw[i]);
  stream_putl(s, ifp->te_min_LSP_bw);
  stream_putl(s, ifp->te_protection_type);
  
  /* Geysers types */
  stream_putl(s, ifp->te_energy_consumption);
  stream_putl(s, ifp->te_max_bw_upgrade);
  stream_putl(s, ifp->te_max_bw_downgrade);
  /* end of Geysers types */

  stream_putl(s, (uint32_t) ifp->adv_rsrv_calendar->count);
  for(ALL_LIST_ELEMENTS(ifp->adv_rsrv_calendar, node, nnode, event))
  {
    stream_putl(s, event->time_stamp);
    for (i=0; i< MAX_BW_PRIORITIES; i++)
      stream_putl(s, event->avail_bw[i]);
  }

  if (ifp->te_swcap == SWCAP_LSC) {
    stream_putl(s, ifp->lambdas_bitmap.base_lambda_label);
    stream_putw(s, ifp->lambdas_bitmap.num_wavelengths);
    stream_putw(s, ifp->lambdas_bitmap.bitmap_size);
    /*check for ifp->lambdas_bitmap_word pointer is inside stream_put function*/
    stream_put (s, ifp->lambdas_bitmap.bitmap_word,
		4 * (ifp->lambdas_bitmap.bitmap_size));
  }

  stream_putl(s, (uint32_t) ifp->te_SRLG_ids->count);
  ...
  switch (ifp->te_TNA_address_type)
  {
    case IPv4:
      stream_putl(s, 32);
      stream_putl(s, ifp->te_TNA_prefix_length);
      stream_putl(s, *(ifp->te_TNA_address));
      break;
    case IPv6:
      stream_putl(s, 128);
      stream_putl(s, ifp->te_TNA_prefix_length);
      stream_putl(s, ifp->te_TNA_address[0]);
      stream_putl(s, ifp->te_TNA_address[1]);
      stream_putl(s, ifp->te_TNA_address[2]);
      stream_putl(s, ifp->te_TNA_address[3]);
      break;
    case NSAP:
      stream_putl(s, 160);
      stream_putl(s, ifp->te_TNA_prefix_length);
      ...
      stream_putl(s, ifp->te_TNA_address[2]);
      stream_putl(s, ifp->te_TNA_address[3]);
      stream_putl(s, ifp->te_TNA_address[4]);
      break;
    case UNNUMBERED:
	    break;
  }

  stream_putl(s, ifp->rem_rc_id);
...
#ifdef GMPLS
int
zsend_te_interface_update(struct zserv     *client,
			  ...
			  int               up_type)
{
	...
	int *data;
	stream_reset(s);
	//zlog_debug("updating interface %s", ifp->name);
	zserv_create_header(s, ZEBRA_INTERFACE_UPDATE);
	stream_put (s, ifp->name, INTERFACE_NAMSIZ);
	stream_putl(s, up_type);
	switch(up_type){
		case METRIC_UPDATE:
			stream_putl(s, ifp->te_metric);
			break;
		case LINK_CLR_UPDATE:
			stream_putl(s, ifp->te_link_color);
			break;
		case SRLGid_UPDATE:
			stream_putl(s, ifp->te_SRLG_ids->count);
			for(ALL_LIST_ELEMENTS(ifp->te_SRLG_ids, node, nnode, data)){
				stream_putl(s, *data);
			}
			break;
		case BW_UPDATE:
			...
			if (ifp->te_swcap == SWCAP_LSC) {
				...
				stream_putw(s, ifp->lambdas_bitmap.bitmap_size);
				/*check for ifp->lambdas_bitmap_word pointer is inside stream_put function*/
				stream_put (s, ifp->lambdas_bitmap.bitmap_word, 4 * (ifp->lambdas_bitmap.bitmap_size));
			}
			break;
		case TNA_UPDATE:
			switch (ifp->te_TNA_address_type)
			{
				...
				case NSAP:
					...
					break;
				default:
					zlog_err("Bad TNA type %d",
						 ifp->te_TNA_address_type);
					assert(0);
			}
			break;
...
#ifdef GMPLS
int
zsend_ospf_router_id_update (struct zserv *client, struct prefix *p, adj_type_t adj, uint32_t powerConsumption)
{
  struct stream *s;
  int blen;
  ...
  stream_put (s, &p->u.prefix, blen);
  stream_putc (s, p->prefixlen);
  stream_putw (s, (uint16_t)(adj));
  stream_putl(s, powerConsumption);
  /* Write packet size. */
  stream_putw_at (s, 0, stream_get_endp (s));
...
zsend_router_id_update (struct zserv *client, struct prefix *p)
{
  ...
  stream_put (s, &p->u.prefix, blen);
  stream_putc (s, p->prefixlen);
  stream_putw (s, (uint16_t) 0);
  zlog_debug("SENDING RID UPDATE MSG");

  zlog_debug("RID ADDRESS: %d", p->u.prefix4.s_addr);
...
{
#if GOTO_LRM
  zlog_debug("Stub Point: LRM must be called here "
	     "to retrieve all TE-links");
  zlog_debug("... to be implemented");
#else
  struct zlistnode *ifnode, *ifnnode;
...
#ifdef GMPLS
static int
zread_ospf_interface_add (struct zserv *client, u_short length, uint32_t type)
{
  zlog_debug("Going to retrieve TE-links from LRM");

  /* Interface information is needed. */
  client->ifinfo = 1;

  /* Call LRM and get TE-links, then send one msg to OSPF */
  /* for each TE-link */
  if (zebra_retrieve_telinks_from_lrm(client, type) < 0) {
    zlog_err("Impossible to get TE-links");
    return -1;
  }
...
#ifdef GMPLS
static int
zread_ospf_router_id_add (struct zserv *client, u_short length, adj_type_t type)
{
  struct prefix p;
  uint32_t      addr = 0;
  uint32_t      powerConsumption = 0;

  zlog_debug("Going to retrieve router id from LRM");

  if (zebra_retrieve_rid_from_lrm(type, &addr, &powerConsumption) < 0) {
    zlog_err("Impossible to get router id");
    return -1;
  }
  ...
  p.family           = AF_INET;
  p.prefixlen        = 32;
  p.u.prefix4.s_addr = addr;

  return zsend_ospf_router_id_update (client, &p, type, powerConsumption);
}
#endif
...
zebra_client_read (struct thread *thread)
{
  ...
  switch (command) 
    ...
    case ZEBRA_ROUTER_ID_ADD:
      zlog_debug("Received ZEBRA_ROUTER_ID_ADD message");
    ...
    case ZEBRA_INTERFACE_ADD:
      zlog_debug("Received ZEBRA_INTERFACE_ADD message");
      zread_interface_add (client, length);
      break;
#ifdef GMPLS
    case ZEBRA_OSPF_UNI_INTERFACE_ADD:
      zlog_debug("Received ZEBRA_OSPF_UNI_INTERFACE_ADD message");
      zread_ospf_interface_add (client, length, OSPF_UNI_CLIENT);
      break;
    case ZEBRA_OSPF_ENNI_INTERFACE_ADD:
      zlog_debug("Received ZEBRA_OSPF_ENNI_INTERFACE_ADD message");
      zread_ospf_interface_add (client, length, OSPF_ENNI_CLIENT);
      break;
    case ZEBRA_OSPF_INNI_INTERFACE_ADD:
      zlog_debug("Received ZEBRA_OSPF_INNI_INTERFACE_ADD message");
      zread_ospf_interface_add (client, length, OSPF_INNI_CLIENT);
      break;
    case ZEBRA_ROUTER_UNI_ID_ADD:
      zlog_debug("Received ZEBRA_ROUTER_UNI_ID_ADD message");
      zread_ospf_router_id_add (client, length, UNI);
      break;
    case ZEBRA_ROUTER_INNI_ID_ADD:
      zlog_debug("Received ZEBRA_ROUTER_INNI_ID_ADD message");
      zread_ospf_router_id_add (client, length, INNI);
      break;
    case ZEBRA_ROUTER_ENNI_ID_ADD:
      zlog_debug("Received ZEBRA_ROUTER_ENNI_ID_ADD message");
      zread_ospf_router_id_add (client, length, ENNI);
      break;
#endif
