  ...
  if (ifp->connected)
      ...
      while ((node = (last ? last->next : listhead (ifp->connected))))
	{
	  ...
	  if (p->family == AF_INET
	    ...
	    {
	      struct zlistnode *anode;
	      struct zlistnode *next;
	      struct zlistnode *first;
	      struct zlist *addr_list;
	      
	      route_unlock_node (rn);
	      addr_list = (struct zlist *) rn->info;
...
if_dump_vty (struct vty *vty, struct interface *ifp)
{
  ...
  for (rn = route_top (zebra_if->ipv4_subnets); rn; rn = route_next (rn))
	...
	continue;
      ...
      for (ALL_LIST_ELEMENTS_RO ((struct zlist *)rn->info, node, connected))
