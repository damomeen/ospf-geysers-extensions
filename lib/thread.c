...
static int
thread_process_fd_debug (struct thread_list *list, fd_set *fdset, fd_set *mfdset)
{
  struct thread *thread;
  struct thread *next;
  int ready = 0;
      ...
      if (FD_ISSET (THREAD_FD (thread), fdset))
        {
          ...
          thread->type = THREAD_READY;
          ready++;
	  zlog_debug("###debug: fd = %d", THREAD_FD (thread));
        }
    }
  return ready;
}
...
thread_fetch (struct thread_master *m, struct thread *fetch)
{
  ...
  while (1)
	...
	timer_wait = timer_wait_bg;
      ...
      tmp_timer.tv_sec = 1;
      timer_wait = &tmp_timer;
      if (timer_wait) {
	      zlog_err("select timeout is now %l sec", timer_wait->tv_sec + timer_wait->tv_usec / 1000000);
      } else {
	      zlog_err("select timeout is not-specified, reverting to a default timeout");
      }
#endif
...
void
thread_call_no_lock (struct thread *thread)
{
  unsigned long realtime, cputime;
  RUSAGE_T ru;
...
/* Execute thread */
struct thread *
funcname_thread_execute_no_lock (struct thread_master *m,
                int (*func)(struct thread *), 
                void *arg,
                int val,
    const char* funcname)
