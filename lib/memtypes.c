...
struct memory_list memory_list_ospf[] =
{
  ...
  { MTYPE_OSPF_INSTATNCE_MAP_ELEMENT,         "OSPF instance mapping"           },
  { MTYPE_OSPF_STR_CHAR,                      "OSPF char"                       },
  { MTYPE_OSPF_GRID_NODE,                     "OSPF grid mode"                  },
  { MTYPE_OSPF_GRID_SITE,                     "OSPF grid site"                  },
  { MTYPE_OSPF_GRID_STORAGE,                  "OSPF grid storage"               },
  { MTYPE_OSPF_GRID_STRAGE_AREA,              "OSPF grid storage area"          },
  { MTYPE_OSPF_GRID_SUBCLUSTER,               "OSPF grid subcluster"            },
  { MTYPE_OSPF_GRID_SUBCLUSTER_CALENDAR,      "OSPF grid subcluster callendar"  },
  { MTYPE_OSPF_GRID_SUBCLUSTER_SOFT_PACKAGE,  "OSPF grid subcluster soft pkg"   },
  { MTYPE_OSPF_GRID_COMPUTING,                "OSPF grid computing"             },
  { MTYPE_OSPF_GRID_COMPUTING_CALENDAR,       "OSPF grid computing callendar"   },
  { MTYPE_OSPF_GRID_SERVICE,                  "OSPF grid service"               },
  { MTYPE_OSPF_GRID_SERVICE_CALENDAR,         "OSPF grid service callendar"     },
  { MTYPE_OSPF_TE_TNA_ADDR_DATA_ELEM,         "TNA address data"                },
  { MTYPE_OSPF_TE_TNA_ADDR_VALUE,             "TNA address"                     },
  { MTYPE_OSPF_TE_SHARED_RISK_L,              "Shared risk link"                },
  { MTYPE_OSPF_TE_AV_WAVE_MASK,               "Available Wavelenght Mask"       },
  { MTYPE_OSPF_TE_AMP_PAR,                    "Amplifiers List Node"            },
  { MTYPE_OSPF_TE_SIGNAL_UNALLOC_TSLOTS,      "Switching Capability Node"       },
  { MTYPE_OSPF_TE_PREFIX_IP4,                 "Node IPv4 Local Prefix"          },
  { MTYPE_OSPF_TE_PREFIX_IP6,                 "Node IPv6 Local Prefix"          },
  { MTYPE_OSPF_TE_ADDRESS_IP4,                "Address IPv4"                    },
  { MTYPE_OSPF_TE_AA_ID,                      "OSPF Area Associated ID Node"    },
  { MTYPE_OSPF_TE_LINKPARAMS,                 "Ospf TE link parameters"         },
  { -1, NULL },
};

#ifdef GMPLS
struct memory_list memory_list_scngws[] =
{
  { MTYPE_SCNGWS,             "SCNGWS structure"    },
  { MTYPE_SCNGWS_PACKET,      "SCNGWS packet"       },
  { MTYPE_SCNGWS_FIFO,        "SCNGWS FIFO queue"       },
  { -1, NULL },
};

struct memory_list memory_list_lrm[] =
{
  { MTYPE_LRM,                "LRM structure"   },
  { MTYPE_LRM_SCNIF,          "LRM SCN intf"      },
  { MTYPE_LRM_CC,             "LRM Control Channel" },
  { MTYPE_LRM_DL,             "LRM Data Link"     },
  { MTYPE_LRM_TEL,            "LRM TE Link"       },
  { MTYPE_LRM_OTHER,          "LRM other"         },
  { MTYPE_LRM_ADJ,            "LRM Adjacency"         },
  { -1, NULL },
};

struct memory_list memory_list_g2pceravty[] =
{
  { MTYPE_G2PCERAVTY,         "G2PCERAVTY structure" },
  { MTYPE_G2PCERAVTY_OTHER,   "G2PCERAVTY other"       },
  { -1, NULL },
};

struct memory_list memory_list_ncc[] =
{
  { MTYPE_NCC,                "NCC structure" },
  { MTYPE_NCC_OTHER,          "NCC other"       },
  { -1, NULL },
};

struct memory_list memory_list_g2rsvptevty[] =
{
  { MTYPE_G2RSVPTEVTY,       "G2RSVPTEVTY structure" },
  { MTYPE_G2RSVPTEVTY_OTHER, "G2RSVPTEVTY other"       },
  { -1, NULL },
};

struct memory_list memory_list_gunigw[] =
{
  { MTYPE_GUNIGW,             "GUNIGW structure"  },
  { MTYPE_GUNIGW_INTF,        "GUNIGW intf"           },
  { MTYPE_GUNIGW_OTHER,       "GUNIGW other"      },
  { -1, NULL },
};

struct memory_list memory_list_tnrc[] =
{
  { MTYPE_TNRC,                "TNRC structure"   },
  { MTYPE_TNRC_OTHER,          "TNRC other"       },
  { -1, NULL },
};
#endif /* GMPLS */
