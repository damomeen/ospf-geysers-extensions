...
#ifdef GMPLS
/** Initialize zebra client.
 * @param zclient pointer to initialized object (struct zclient)
 * @param redist_default unwanted redistribute route type
 */
void
zclient_init (struct zclient *zclient, int redist_default, int type_mask)
{
  int i;
...
/** RESTART (stop and start zebra client services. */
void
zclient_reset (struct zclient *zclient)
{
  zclient_stop (zclient);
#ifdef GMPLS
  zclient_init (zclient, zclient->redist_default, 0);
#else
...
int
zclient_socket(struct zclient *zclient)
{
  int sock;
  int ret;
...
zclient_start (struct zclient *zclient)
{
  ...
  zclient->sock = zclient_socket (zclient);
#else
...
#ifdef GMPLS
  if (zclient->client_type & OSPF_UNI_CLIENT) {
      zebra_message_send (zclient, ZEBRA_OSPF_UNI_INTERFACE_ADD);
      zebra_message_send (zclient, ZEBRA_ROUTER_UNI_ID_ADD);
  }
  if (zclient->client_type & OSPF_INNI_CLIENT) {
      zebra_message_send (zclient, ZEBRA_OSPF_INNI_INTERFACE_ADD);
      zebra_message_send (zclient, ZEBRA_ROUTER_INNI_ID_ADD);
  }
  if (zclient->client_type & OSPF_ENNI_CLIENT) {
      zebra_message_send (zclient, ZEBRA_OSPF_ENNI_INTERFACE_ADD);
      zebra_message_send (zclient, ZEBRA_ROUTER_ENNI_ID_ADD);
  }
#else		  
 ...
 * @param stream with zprotocol data
 * @param rid [OUT] router ID
 * @return adjacency type
 */
uint16_t
zebra_router_id_update_read (struct stream *s, struct prefix *rid, uint32_t* energyConsumption)
{
  int plen;
  ...
  uint16_t adj = stream_getw(s);
  *energyConsumption = stream_getl(s);
  return adj;
}
 ...
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 *
GMPLS extension (if defined GMPLS)
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        adj_type             | ospf_instance                   |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_local_id                                            |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_remote_id                                           |
 ...
 * |        te_max_res_bw                                          |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_avail_bw_per_prio (1)                               |
 * |         ...                                                   |
 * |        te_avail_bw_per_prio (MAX_BW_PRIORITIES)               |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_max_LSP_bw (1)                                      |
 * |         ...                                                   |
 * |        te_max_LSP_bw (MAX_BW_PRIORITIES)                      |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_min_LSP_bw                                          |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        te_protection_type                                     |
 ...
 * |        SRLG link no 1                                         |
 * |        ...                                                    |
 * |        SRLG link no (length of SRLG list)                     |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        TNA address length (bits)                              |
 * |        tna address prefix length                              |
 * |        tna address                                            |
 * |        ....                                                   |
 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 * |        remote RC_ID                                           |
...
/*Read the TE params recived from LRM: use another message to update once the interface and associated TE
 * params are created*/
#ifdef GMPLS
  ifp->flags |= IFF_POINTOPOINT;
  ifp->adj_type           = stream_getw(s);
  ifp->ospf_instance      = stream_getw(s);
  ifp->te_local_id        = stream_getl(s);
  ifp->te_remote_id       = stream_getl(s);
  ifp->te_remote_node_id  = stream_getl(s);
  ifp->te_metric          = stream_getl(s);
  ifp->te_link_color      = stream_getl(s);
  ifp->te_swcap           = stream_getl(s);
  ifp->te_swcap_options   = stream_getl(s);
  ifp->te_enctype         = stream_getl(s);
  ifp->te_max_bw          = stream_getl(s);
  ifp->te_max_res_bw      = stream_getl(s);
  for(i = 0; i < MAX_BW_PRIORITIES; i++)
    ifp->te_avail_bw_per_prio[i] = stream_getl(s);

  for(i = 0; i < MAX_BW_PRIORITIES; i++)
    ifp->te_max_LSP_bw[i] = stream_getl(s);

  ifp->te_min_LSP_bw = stream_getl(s);
  ifp->te_protection_type = stream_getl(s);

  /* Geysers types */
  ifp->te_energy_consumption = stream_getl(s);
  ifp->te_max_bw_upgrade = stream_getl(s);
  ifp->te_max_bw_downgrade = stream_getl(s);
  /* end of Geysers types */

  counter = stream_getl(s);
  list_delete_all_node(ifp->adv_rsrv_calendar);
  for(i = 0; i < counter; i++){
	  event = (calendar_event_t *) malloc(sizeof(*event));
	  event->time_stamp = stream_getl(s);
	  for(int j = 0; j < MAX_BW_PRIORITIES; j++) {
		  event->avail_bw[j] = stream_getl(s);
	  }
	  zlog_debug("added event at UNIX time %d to interface %s", event->time_stamp, ifname_tmp);
	  listnode_add(ifp->adv_rsrv_calendar, event);
  }

  if (ifp->te_swcap == SWCAP_LSC) {
	  ifp->lambdas_bitmap.base_lambda_label = stream_getl(s);
	  ifp->lambdas_bitmap.num_wavelengths   = stream_getw(s);
	  ifp->lambdas_bitmap.bitmap_size       = stream_getw(s);
	  size_t size;
	  size = 4 * (ifp->lambdas_bitmap.bitmap_size);
	  if (!ifp->lambdas_bitmap.bitmap_word) {
		  ifp->lambdas_bitmap.bitmap_word = (uint32_t *) malloc(size);
	  }
	  stream_get(ifp->lambdas_bitmap.bitmap_word, s, size);  /* TODO Adam What about bit order ? */
  }

  counter = stream_getl(s);
  list_delete_all_node(ifp->te_SRLG_ids);
  for(i = 0; i < counter; i++) {
	  srlg_id = (uint32_t *)malloc(sizeof(uint32_t));
	  *srlg_id  = stream_getl(s);
	  listnode_add(ifp->te_SRLG_ids, srlg_id);
  }

  int tna_address_len = stream_getl(s);
  ifp->te_TNA_prefix_length = stream_getl(s);
  if (ifp->te_TNA_address != NULL)
    free(ifp->te_TNA_address);

  switch (tna_address_len)
  {
    case 32:
      ifp->te_TNA_address = (u_int32_t *)malloc(4);
      ifp->te_TNA_address[0] = stream_getl(s);
      break;
    case 128:
      ifp->te_TNA_address = (u_int32_t *)malloc(16);
      ifp->te_TNA_address[0] = stream_getl(s);
      ifp->te_TNA_address[1] = stream_getl(s);
      ifp->te_TNA_address[2] = stream_getl(s);
      ifp->te_TNA_address[3] = stream_getl(s);
      break;
    case 160:
      ...
      ifp->te_TNA_address[2] = stream_getl(s);
      ifp->te_TNA_address[3] = stream_getl(s);
      ifp->te_TNA_address[4] = stream_getl(s);
      break;
    default:
      zlog_warn("tna address update wrong address length %d", tna_address_len);
  }

  ifp->rem_rc_id = stream_getl(s);

  ret = (*ifp->if_set_te_params)(ifp);
  ...
  {
    zlog_debug("returned 0 from if_set_te_params in zebra_interface_add_read, exiting");
    exit(0);
  }
}
...
struct interface *
zebra_interface_update_read (struct stream *s)
{
	int tna_address_len;
	...
	/* Read interface name. */
  	stream_get(ifname_tmp, s, INTERFACE_NAMSIZ);
	zlog_debug("updating interface %s", ifname_tmp);
  	/* Lookup/create interface by name. */
  	ifp = if_get_by_name_len (ifname_tmp, strnlen(ifname_tmp, INTERFACE_NAMSIZ));
	update_type = stream_getl(s);
	assert(ifp);
	...
	switch(update_type){
		case 0:    //BW_UPDATE
			{
				...
				zlog_debug("updating the BW parameters of interface %s", ifname_tmp);
				int i = 0;
				ifp->te_max_bw = stream_getl(s);
				ifp->te_max_res_bw = stream_getl(s);
				ifp->te_min_LSP_bw = stream_getl(s);
				...
				{
					ifp->te_avail_bw_per_prio[i] = stream_getl(s);
					zlog_debug("te_avail_bw_per_prio[%d] = 0x%x", i,ifp->te_avail_bw_per_prio[i]);
				}
				for(i = 0; i < MAX_BW_PRIORITIES; i++)
				{
					ifp->te_max_LSP_bw[i] = stream_getl(s);
					zlog_debug("te_max_LSP_bw[%d] = 0x%x", i, ifp->te_max_LSP_bw[i]);
				}
				counter = stream_getl(s);
				zlog_debug("number of advance reservation calendar "
					   "events for this te-link is %d", counter);
				list_delete_all_node(ifp->adv_rsrv_calendar);
				for(size_t i = 0; i < counter; i++){
					...
					zlog_debug("added event at UNIX time %d "
						   "to interface %s", event->time_stamp, ifname_tmp);
					listnode_add(ifp->adv_rsrv_calendar, event);
				}
				if (ifp->te_swcap == SWCAP_LSC) {
					ifp->lambdas_bitmap.base_lambda_label = stream_getl(s);
					...
					if (!ifp->lambdas_bitmap.bitmap_word) {
						ifp->lambdas_bitmap.bitmap_word = (uint32_t *) malloc(size);
						// TODO Adam, release momory before. Releasing memory makes problems, because te_link_t pointer shows on this memory address
					}
					stream_get(ifp->lambdas_bitmap.bitmap_word, s, size);
				}
			}
			break;
		case 1:  //SRLGid_UPDATE
			{
				zlog_debug("updating the SRLGids of interface %s", ifname_tmp);
				int counter = 0;
				int i = 0;
				int *srlg_id = NULL;
				counter = stream_getl(s);
				if(ifp->te_SRLG_ids){
		  			list_delete(ifp->te_SRLG_ids);
		 	 		ifp->te_SRLG_ids = list_new();
  				}
  				for(i = 0; i < counter; i++){
	  				srlg_id = (int *)malloc(sizeof(int));
	  				*srlg_id  = stream_getl(s);
	  				zlog_debug("added SRLG id %d to interface %s", *srlg_id, ifname_tmp); 
	  				listnode_add(ifp->te_SRLG_ids, srlg_id);
  				}
			}
			break;
		case 2:  //METRIC_UPDATE
			zlog_debug("updating the metric of interface %s", ifname_tmp);
			ifp->te_metric = stream_getl(s);
			break;
		case 3: //LINK_CLR_UPDATE
			zlog_debug("updating the link color of interface %s", ifname_tmp);
			ifp->te_link_color = stream_getl(s);
			break;
		case 4: //TNA_UPDATE
			tna_address_len = stream_getl(s);
			ifp->te_TNA_prefix_length = stream_getl(s);
			if (ifp->te_TNA_address != NULL)
				free(ifp->te_TNA_address);
		...
		case 5: //PROTECTION_UPDATE
			ifp->te_protection_type = stream_getl(s);
			zlog_debug("updating protection type %d", ifp->te_protection_type);
			break;
		case 6: //ENERGY_UPDATE
			ifp->te_energy_consumption = stream_getl(s);
			zlog_debug("updating energy consumption %x", ifp->te_energy_consumption);
			break;
		case 7: //BW_REPLANNING_UPDATE
			ifp->te_max_bw_upgrade = stream_getl(s);
            ifp->te_max_bw_downgrade = stream_getl(s);
			zlog_debug("updating bandwidth replanning %x - %x", ifp->te_max_bw_upgrade, ifp->te_max_bw_downgrade);
			break;
		default:
			zlog_debug("unknow kind of interface update");
			break;
	}
	ret = (*ifp->if_set_te_params)(ifp);
	if(!ret)
		zlog_debug("could not update inteface %s", ifname_tmp);
	return ifp;
}
...
zclient_read (struct thread *thread)
{
  ...
  if (zclient_debug)
    zlog_debug("zclient 0x%x command: 0x%x", zclient, command);
  ...
  switch (command)
    ...
    case ZEBRA_INTERFACE_UPDATE:
      if (zclient->interface_te_update)
	ret = (*zclient->interface_te_update) (command, zclient, length);
      break;
#ifdef GMPLS
#if !HAVE_OMNIORB
    case LRM_BEGIN_SYNC:
      if (zclient->begin_sync) {
	      ret = (*zclient->begin_sync) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_END_SYNC:
      if (zclient->end_sync) {
	ret = (*zclient->end_sync) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_SCNIF_ADD:
      if (zclient->scnif_add) {
	ret = (*zclient->scnif_add) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_SCNIF_DELETE:
      if (zclient->scnif_delete) {
	ret = (*zclient->scnif_delete) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_TEL_CC_ASSOC_ADD:
      if (zclient->tel_cc_assoc_add) {
	ret = (*zclient->tel_cc_assoc_add) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_TEL_CC_ASSOC_DELETE:
      if (zclient->tel_cc_assoc_delete) {
        ret = (*zclient->tel_cc_assoc_delete) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_TEL_CC_ASSOC_UPDATE:
      if (zclient->tel_cc_assoc_update) { 
        ret = (*zclient->tel_cc_assoc_update) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_CC_SCNIF_ASSOC_ADD:
      if (zclient->cc_scnif_assoc_add) {
	ret = (*zclient->cc_scnif_assoc_add) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_CC_SCNIF_ASSOC_DELETE:
      if (zclient->cc_scnif_assoc_delete) {
	ret = (*zclient->cc_scnif_assoc_delete) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_CC_SCNIF_ASSOC_UPDATE:
      if (zclient->cc_scnif_assoc_update) {
	ret = (*zclient->cc_scnif_assoc_update) (command, zclient, length);
	zebra_message_send (zclient, ret);
	zlog_debug ("Message sent to LRM (command %d)", ret);
      }
      break;
    case LRM_SYNC_FATAL_ERROR:
      if (zclient->sync_fatal_error) {
	ret = (*zclient->sync_fatal_error) (command, zclient, length);
      }
      break;
#endif /* !HAVE_OMNIORB */
#endif /*GMPLS*/
