...
cmp_node (const void *p, const void *q)
{
  ...
  return strcmp (a->sstring, b->sstring);
}
...
vector
cmd_make_strvec (const char *sstring)
{
  const char *cp, *start;
  char *token;
  int strlen;
...
static char *
cmd_desc_str (const char **sstring)
{
  const char *cp, *start;
  char *token;
  int strlen;
...
/* New string vector. */
static vector
cmd_make_descvec (const char *sstring, const char *descstr)
{
  int multiple = 0;
  const char *sp;
  char *token;
...
install_element (enum node_type ntype, struct cmd_element *cmd)
{
  ...
  cmd->strvec = cmd_make_descvec (cmd->sstring, cmd->doc);
	...
	if (command && index >= vector_active (strvec))
	...
	else
	  {
	    ...
	    if (command == NULL && index == vector_active (strvec))
	      {
		sstring = "<cr>";
		if (!desc_unique_string (matchvec, sstring))
		  vector_set (matchvec, &desc_cr);
	      }
	    else
	      {
		...
		for (j = 0; j < vector_active (descvec); j++)
		  if ((desc = vector_slot (descvec, j)))
		    {
		      sstring = cmd_entry_function_desc (command, desc->cmd);
		      if (sstring)
			{
			  /* Uniqueness check */
			  if (!desc_unique_string (matchvec, sstring))
	      ...
	      if ((desc = vector_slot (descvec, j)))
		{
		  if ((sstring = 
		       cmd_entry_function (vector_slot (vline, index),
					   desc->cmd)))
		    if (cmd_unique_string (matchvec, sstring))
		      vector_set (matchvec, XSTRDUP (MTYPE_TMP, sstring));
		}
	  }
      }
       ...
       "Exit current mode and down to previous mode\n")
{
  switch (vty->node)
    ...
    case RMAP_NODE:
    case VTY_NODE:
    case OSPF_TE_LINK_NODE:
    case XXX_NODE:
    case G2RSVPTE_NODE:
    case GUNIGW_NODE:
    case TNRC_NODE:
    case G2PCERAVTY_NODE:
    case NCC_NODE:
    case G2RSVPTEVTY_NODE:
    case LRM_NODE:
    case SCNGWS_NODE:
      vty->node = CONFIG_NODE;
      break;
       ...
       "Write to terminal\n")
{
  ...
  if (vty->type == VTY_SHELL_SERV)
    {
	vty_out(vty, "write config ro terminal%s", VTY_NEWLINE);
      for (i = 0; i < vector_active (cmdvec); i++){
	vty_out(vty, "write config ro terminal%s", VTY_NEWLINE);
