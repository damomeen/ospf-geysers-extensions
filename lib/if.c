...
if_create (const char *name, int namelen)
{
  ...
  #ifdef GMPLS
  ifp->adv_rsrv_calendar      = list_new();
  ifp->adv_rsrv_calendar->del = delete_calendar_event;
  memset(&(ifp->lambdas_bitmap), 0, sizeof(ifp->lambdas_bitmap));
  ifp->te_SRLG_ids = list_new();
  ifp->te_SRLG_ids->del = del_te_SRLG_id;
  ifp->te_TNA_address = NULL;
  #endif /*GMPLS*/
  if (if_master.if_new_hook)
    (*if_master.if_new_hook) (ifp);
  //here  should be add the call-back to populate the te-link
  if (if_master.if_set_te_params)
	ifp->if_set_te_params = if_master.if_set_te_params;
  return ifp;
}

struct interface *
if_create_level1 (const char *name, int namelen)
{
  struct interface *ifp;

  ifp = XCALLOC (MTYPE_IF, sizeof (struct interface));
  ifp -> adj_type      = ENNI;
  ifp -> ospf_instance = ENNI;
  ifp -> ifindex       = IFINDEX_INTERNAL;
...
#ifdef GMPLS
  case TE_SET_PARAMS:
    if_master.if_set_te_params = func;
    break;
#endif
  default:
    break;
