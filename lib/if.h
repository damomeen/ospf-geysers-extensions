...
#define UNI  2
#endif /* INNI */
  u_int16_t     adj_type;                                 /** interface adjecency */
  u_int16_t     ospf_instance;                            /** ospf instance */
  u_int32_t     te_local_id;                              /** local  link id,                            stored in NO */
  u_int32_t     te_remote_id;                             /** remote link id,                            stored in NO */
  u_int32_t     te_remote_node_id;                        /** remote node id                             stored in HO */
  u_int32_t     te_metric;                                /** metric                                     stored in HO */
  u_int32_t     te_link_color;                            /** link color                                 stored in HO */
  sw_cap_t      te_swcap;                                 /** switching capability                       stored in HO */
  u_int32_t     te_swcap_options;                         /** options that depends of sw. capability     stored in HO */
  enc_type_t    te_enctype;                               /** the encoding type of this data link        stored in HO */
  u_int32_t     te_max_bw;                                /** maximum bandwidth                          stored in HO */
  u_int32_t     te_max_res_bw;                            /** maximum reservable bandwidth               stored in HO */
  u_int32_t     te_avail_bw_per_prio[MAX_BW_PRIORITIES];  /**                                            stored in HO */
  u_int32_t     te_max_LSP_bw[MAX_BW_PRIORITIES];         /**                                            stored in HO */
  u_int32_t     te_min_LSP_bw;

  u_int8_t      te_protection_type;                       /** Protection type */
   
  /* Geysers types */
  u_int32_t     te_energy_consumption;                    /** Energy consumption                         stored in HO */
  u_int32_t     te_max_bw_upgrade;                        /** Max bandwidth upgrade                      stored in HO */
  u_int32_t     te_max_bw_downgrade;                      /** Max bandwidth downgrade                    stored in HO */
  /* end of Geysers types */

  struct zlist * adv_rsrv_calendar;;                      /** advance reservation calendar of events     stored in HO */
  wdm_link_lambdas_bitmap_t lambdas_bitmap;               /** wavelength bitmask                         stored in HO */
  struct zlist  *te_SRLG_ids;                             /** shared link list group                     stored in HO */

  addr_type_t   te_TNA_address_type;
  u_int32_t     te_TNA_prefix_length;
  u_int32_t     *te_TNA_address;
  u_int32_t     rem_rc_id;                                /** remote RC ID                               stored in NO */

  /*the callback that gets called after the interface details are received by ospfd*/
  int (*if_set_te_params) (struct interface *);
#endif
...
extern struct interface *if_create_level1 (const char *name, int namelen);
...
extern struct cmd_element interface_global_cmd;
