...
/* TODO move it into the file g2mpls_types.h */
typedef enum interf_side {
	 NETWORK = 0,
	 CLIENT  = 1,
} interf_side_t;
...
#define DEFAULT_ORIGINATE_ALWAYS	2
  ...
  struct zlist *vlinks;                 /* Configured Virtual-Links. */
  struct zlist *areas;                  /* OSPF areas. */
  struct route_table *nbr_nbma;
  struct ospf_area *backbone;           /* Pointer to the Backbone Area. */

  struct zlist *oiflist;                /* ospf interfaces */
...
#ifdef HAVE_OPAQUE_LSA
  struct zlist *opaque_lsa_self;		/* Type-11 Opaque-LSAs */
#endif /* HAVE_OPAQUE_LSA */
  ...
  struct zlist *maxage_lsa;              /* List of MaxAge LSA for deletion. */
...
#define OSPF_LSA_REFRESHER_SLOTS ((OSPF_LS_REFRESH_TIME + \
  ...
  {
    u_int16_t index;
    struct zlist *qs[OSPF_LSA_REFRESHER_SLOTS];
  } lsa_refresh_queue;
...
#define OSPF_LSA_REFRESH_INTERVAL_DEFAULT 10
  ...
  adj_type_t instance;
  interf_side_t interface_side;
  
  uint8_t read_tna;

  struct interface *rc_interface;
};
...
#ifdef HAVE_OPAQUE_LSA
  struct zlist *opaque_lsa_self;		/* Type-10 Opaque-LSAs */
#endif /* HAVE_OPAQUE_LSA */
...
#define EXPORT_NAME(A)  (A)->export1.name
#define EXPORT_LIST(A)  (A)->export1.list
...
extern struct ospf *ospf_inni_lookup (void);
extern struct ospf *ospf_enni_lookup (void);
extern struct ospf *ospf_uni_lookup (void);
extern struct ospf *ospf_get (void);
extern struct ospf *ospf_enni_get (void);
extern struct ospf *ospf_inni_get (void);
extern struct ospf *ospf_uni_get (void);
