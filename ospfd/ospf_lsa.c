...
ospf_lsa_unlock (struct ospf_lsa **lsa)
{
  ...
  if ((*lsa)->lock < 0)
    zlog_err("[ERR] (*lsa)->lock = %d", (*lsa)->lock);
  assert ((*lsa)->lock >= 0);
...
ospf_nbr_lookup_ptop (struct ospf_interface *oi)
{
  ...
  if (ospf_nbr_count (oi, 0) > 1)
    zlog_warn ("[WRN] Point-to-Point link has more than 1 neighobrs.");
  ...
  if (STREAM_WRITEABLE(s) < OSPF_ROUTER_LSA_LINK_SIZE)
    {
      ...
      if (ret == OSPF_MAX_LSA_SIZE)
        {
          zlog_warn ("[WRN] %s: Out of space in LSA stream, left %ld, size %ld",
                     __func__, (long int) STREAM_REMAIN (s), (long int) STREAM_SIZE (s));
          return 0;
        }
    }
...
ospf_router_lsa_new (struct ospf_area *area)
{
  ...
  if ( (new = ospf_lsa_new ()) == NULL)
    {
      zlog_err ("[ERR] %s: Unable to create new lsa", __func__);
      return NULL;
    }
...
ospf_router_lsa_originate (struct ospf_area *area)
{
  ...
  if ( (new = ospf_router_lsa_new (area)) == NULL)
    {
      zlog_err ("[ERR] %s: ospf_router_lsa_new returned NULL", __func__);
      return NULL;
    }
...
ospf_router_lsa_timer (struct thread *t)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[router-LSA]: (router-LSA Refresh expire)");
...
ospf_router_lsa_update_timer (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
    zlog_debug ("[DBG] Timer[router-LSA Update]: (timer expire)");
...
ospf_network_lsa_new (struct ospf_interface *oi)
{
  ...
  if ( (new = ospf_lsa_new ()) == NULL)
    {
      zlog_err ("[ERR] %s: ospf_lsa_new returned NULL", __func__);
      return NULL;
    }
...
static struct ospf_lsa *
ospf_summary_asbr_lsa_new (struct ospf_area *area, struct prefix *p, u_int32_t metric, struct in_addr id)
{
  struct stream *s;
  struct ospf_lsa *new;
  struct lsa_header *lsah;
 ...
 */
struct ospf_lsa *
ospf_summary_asbr_lsa_originate (struct prefix_ipv4 *p, u_int32_t metric, struct ospf_area *area)
{
  struct ospf_lsa *new;
  struct in_addr id;
  ...
  if ((rt = EXTERNAL_INFO (type)))
	...
	if (!is_prefix_default ((struct prefix_ipv4 *)&ei->p))
	  if (!ospf_external_lsa_originate (ospf, ei))
	    zlog_warn ("[WRN] LSA: AS-external-LSA was not originated.");
		      ...
		      struct ospf_lsdb *lsdb, struct ospf_lsa *lsa)
{
  ...
  if (!lsdb)
    {
      zlog_warn ("[WRN] %s: Called with NULL lsdb!", __func__);
      if (!lsa)
        zlog_warn ("[WRN] %s: and NULL LSA!", __func__);
      else
        zlog_warn ("[WRN] LSA[Type%d:%s]: not associated with LSDB!",
                   lsa->data->type, inet_ntoa (lsa->data->id));
      return;
    }
	...
	if (lsa->lsdb)
	  {
	    ...
	    ospf_lsdb_delete (lsa->lsdb, lsa);
          ...
          zlog_warn ("[WRN] %s: LSA[Type%d:%s] ospf instance %d: No associated LSDB!", __func__,
                     lsa->data->type, inet_ntoa (lsa->data->id), ospf->instance);
      }
...
#else
  /* FIXME decision about ospf_inni/enni_lookup is taken according to the pointer to ospf (area->ospf) */ 
  struct ospf *ospf;
  switch (area->ospf->instance)
  {
    case INNI:
...
#else
      thread_execute_no_lock (master, ospf_maxage_lsa_remover, ospf, 0);
#endif /* HAVE_OMNIORB */
    }
...
ospf_refresher_unregister_lsa (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if (lsa->refresh_list >= 0)
    {
      struct zlist *refresh_list = ospf->lsa_refresh_queue.qs[lsa->refresh_list];
...
{
  struct zlist *refresh_list;
  struct zlistnode *node, *nnode;
  struct ospf *ospf = THREAD_ARG (t);
  struct ospf_lsa *lsa;
  int i;
  struct zlist *lsa_to_refresh = list_new ();
