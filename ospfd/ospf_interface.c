...
int conf_ospf_hello_interval = OSPF_HELLO_INTERVAL_DEFAULT;
...
/** Restore an interface to its pre UP state Used from ism_interface_down only */
void
...
#else
  OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_KillNbr);
#endif
...
ospf_if_new_hook (struct interface *ifp)
{
  ...
  IF_DEF_PARAMS (ifp)->v_hello = conf_ospf_hello_interval;
...
int
createRC (struct in_addr RCaddress, int addr_len)
{
  struct ospf *ospf = ospf_enni_get();
  SET_FLAG (ospf->opaque, OPAQUE_OPERATION_READY_BIT); /*quagga hacking*/
...
//  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("createRC:  creating pseudo zebra interface");

  if (ospf->rc_interface == NULL)
  {
    vi = if_create_level1 (ifname, strnlen(ifname, sizeof(ifname)));
    ospf->rc_interface  = vi;
  }
  else
    vi = ospf->rc_interface;
  ...
  p = prefix_ipv4_new ();
  p->family = AF_INET;
  p->prefix.s_addr = RCaddress.s_addr;
  p->prefixlen = addr_len;
  ...
  {
    if (IS_DEBUG_OSPF_EVENT)
      zlog_debug ("createRC: Alarm: OSPF int structure is not created");
    return 0;
  }
  voi->connected = co;
  /* voi->vl_data = vl_data; */
  voi->ifp->mtu = OSPF_VL_MTU;
  voi->type = OSPF_IFTYPE_BROADCAST;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("createRC: set associated area to the backbone");
  ...
  /* force UP interface */
  vi -> flags        |= IFF_UP;
  vi -> flags        |= IFF_RUNNING;
  vi -> bandwidth     = 10000;
  vi -> status        = 7;

  OSPF_ISM_EVENT_EXECUTE_NO_LOCK (voi, ISM_InterfaceUp);
...
ospf_vl_set_params (struct ospf_vl_data *vl_data, struct vertex *v)
{
  ...
  if (vp && vp->backlink >= 0)
  ...
  else
    {
       ...
       */
      zlog_warn ("[WRN] ospf_vl_set_params: No backlink for %s!",
                 vl_data->vl_oi->ifp->name);
...
ospf_full_virtual_nbrs (struct ospf_area *area)
{
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("[DBG] Counting fully adjacent virtual neighbors in area %s",
		 inet_ntoa (area->area_id));
      zlog_debug ("[DBG]   there are %d of them", area->full_vls);
    }
...
void
ospf_crypt_key_add (struct zlist *crypt, struct crypt_key *ck)
{
  listnode_add (crypt, ck);
}

struct crypt_key *
ospf_crypt_key_lookup (struct zlist *auth_crypt, u_char key_id)
{
  struct zlistnode *node;
  struct crypt_key *ck;
...
int
ospf_crypt_key_delete (struct zlist *auth_crypt, u_char key_id)
{
  struct zlistnode *node, *nnode;
  struct crypt_key *ck;
...
ospf_if_init ()
{
  ...
  if_add_hook (TE_SET_PARAMS, read_te_params_from_ifp);
}
