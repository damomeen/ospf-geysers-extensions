...
void
del_mytype_instance_map_element(void *val)
{
  XFREE(MTYPE_OSPF_INSTATNCE_MAP_ELEMENT, val);
  return;
}

uint32_t
lookup_from_map(struct zlist *inst_list, struct in_addr adv_router, uint32_t old_instance_no)
{
  struct zlistnode            *node;
  struct instance_map_element *element;

  // TODO Damian: repair this
  //for(ALL_LIST_ELEMENTS_RO(inst_list, node, element))
  //{
  //  if ((IPV4_ADDR_SAME(&adv_router, &element->adv_router)) && (element->old_instance_no == old_instance_no))
  //  {
  //    return element->new_instance_no;
  //  }
  //s}
  return 0;
}
...
uint32_t
get_from_map(struct zlist *inst_list, struct in_addr adv_router, uint32_t old_instance_no, uint32_t(*get_instance)())
{
  struct zlistnode            *node;
  struct instance_map_element *element;
  ...
  //element = XMALLOC(MTYPE_OSPF_INSTATNCE_MAP_ELEMENT, sizeof(struct instance_map_element));
  //element->adv_router        = adv_router;
  //element->old_instance_no   = old_instance_no;
  //element->new_instance_no   = get_instance();
  //listnode_add(inst_list, element);
  //return element->new_instance_no;
  return 0;
}
...
//#ifdef HAVE_OSPF_TE
  zlog_err("[ERR] in ospf_opaque_init: init ospf-te: %s", __FILE__);
  if (ospf_te_init () != 0)
    exit (1);
  zlog_err("[ERR] in ospf_opaque_init: ospf-te initialized: %s", __FILE__);
//#endif /* HAVE_OSPF_TE */
...
static struct zlist *ospf_opaque_wildcard_funclist;
static struct zlist *ospf_opaque_type9_funclist;
static struct zlist *ospf_opaque_type10_funclist;
static struct zlist *ospf_opaque_type11_funclist;
...
ospf_get_opaque_funclist (u_char lsa_type)
{
  ...
  switch (lsa_type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_get_opaque_funclist: Unexpected LSA-type(%u)", lsa_type);
      break;
    }
  return funclist;
}
...
/**
 * Createn new function table. Adds this fontion to the on of the lists:
 * - ospf_opaque_wildcard_funclist  
 * - ospf_opaque_type9_funclist OSPF_OPAQUE_LINK_LSA
 * - ospf_opaque_type10_funclist OSPF_OPAQUE_AREA_LSA,
 * - ospf_opaque_type11_funclist OPAQUE_AS_LSA
 * Function sets the function table's pointers to the specyfied function.
 * @param lsa_type,
 * @param opaque_type,
 * @param (*new_if_hook) (struct interface *ifp) - adding new interface
 * @param  int (* del_if_hook)(struct interface *ifp) - removing  interface
 * @param  void (* ism_change_hook)(struct ospf_interface *oi, int old_status),
 * @param  void (* nsm_change_hook)(struct ospf_neighbor *nbr, int old_status),
 * @param  void (* config_write_router)(struct vty *vty) writing router configuration
 * @param  void (* config_write_if    )(struct vty *vty) struct interface *ifp), writing interface configuration
 * @param  void (* config_write_debug )(struct vty *vty)
 * @param  void (* show_opaque_info   )(struct vty *vty struct ospf_lsa *lsa), writing opaque information
 * @param  int  (* lsa_originator)(void *arg)
 * @param  void (* lsa_refresher )(struct ospf_lsa *lsa)
 * @param  int (* new_lsa_hook)(struct ospf_lsa *lsa)
 * @param  int (* del_lsa_hook)(struct ospf_lsa *lsa)
 * @return result
 */
int
  ...
  int (* del_lsa_hook)(struct ospf_lsa *lsa))
{
  ...
  if ((funclist = ospf_get_opaque_funclist (lsa_type)) == NULL)
    {
      zlog_warn ("[WRN] ospf_register_opaque_functab: Cannot get funclist"
                 " for Type-%u LSAs?",
                 lsa_type);
      goto out;
    }
  else
    {
      ...
      for (ALL_LIST_ELEMENTS (funclist, node, nnode, functab))
        if (functab->opaque_type == opaque_type)
          {
            zlog_warn ("[WRN] ospf_register_opaque_functab: Duplicated entry?:"
  ...
  if ((new = XCALLOC (MTYPE_OSPF_OPAQUE_FUNCTAB,
    ...
    {
      zlog_warn ("[WRN] ospf_register_opaque_functab: XMALLOC: %s",
                 safe_strerror (errno));
      goto out;
    }
                               ...
                               struct ospf_lsa *new)
{
  ...
  if ((oipt = XCALLOC (MTYPE_OPAQUE_INFO_PER_TYPE,
    ...
    {
      zlog_warn ("[WRN] register_opaque_info_per_type: XMALLOC: %s", safe_strerror (errno));
      goto out;
    }
  ...
  switch (new->data->type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] register_opaque_info_per_type: Unexpected LSA-type(%u)", new->data->type);
      free_opaque_info_per_type ((void *) oipt);
      oipt = NULL;
      goto out; /* This case may not exist. */
    }
...
free_opaque_info_per_type (void *val)
{
  ...
  switch (oipt->lsa_type)
    ...
    default:
      zlog_warn ("[WRN] free_opaque_info_per_type: Unexpected LSA-type(%u)", oipt->lsa_type);
  ...
  switch (lsa->data->type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      if ((oi = lsa->oi) != NULL)
        ...
        zlog_warn ("[WRN] Type-9 Opaque-LSA: Reference to OI is missing?");
      break;
    case OSPF_OPAQUE_AREA_LSA:
      if ((area = lsa->area) != NULL)
        listtop = area->opaque_lsa_self;
      else
        zlog_warn ("[WRN] Type-10 Opaque-LSA: Reference to AREA is missing?");
      break;
    case OSPF_OPAQUE_AS_LSA:
      top = ospf_lookup ();
      if ((area = lsa->area) != NULL && (top = area->ospf) == NULL)
        {
          zlog_warn ("[WRN] Type-11 Opaque-LSA: Reference to OSPF is missing?");
          break; /* Unlikely to happen. */
        }
      listtop = top->opaque_lsa_self;
      break;
    default:
      zlog_warn ("[WRN] lookup_opaque_info_by_type: Unexpected LSA-type(%u)", lsa->data->type);
      break;
    }
                             ...
                             struct ospf_lsa *new)
{
  ...
  if ((oipi = XCALLOC (MTYPE_OPAQUE_INFO_PER_ID,
    ...
    {
      zlog_warn ("[WRN] register_opaque_info_per_id: XMALLOC: %s", safe_strerror (errno));
      goto out;
    }
                          ...
                          struct ospf_lsa *lsa)
{
  ...
  if (oipt == NULL)
  {
    ...
    zlog_err("[ERR] lookup_opaque_info_by_id: oipt = NULL, opaque id %s", inet_ntoa(temp));
    return NULL;
  }
...
out:
/*  zlog_debug("register_opaque_lsa: %x", oipi); */
  return oipi;
}
...
{
   zlog_debug ("TE capability: OFF -> ON");
...
ospf_opaque_register_vty (void)
{
  ...
  install_element (OSPF_NODE, &capability_te_cmd);
  return;
}
...
static int
opaque_lsa_new_if_callback (struct zlist *funclist, struct interface *ifp)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  int rc = -1;
...
static int
opaque_lsa_del_if_callback (struct zlist *funclist, struct interface *ifp)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  int rc = -1;
...
static void
opaque_lsa_ism_change_callback (struct zlist *funclist,
...
static void
opaque_lsa_nsm_change_callback (struct zlist *funclist,
...
static void
opaque_lsa_config_write_router_callback (struct zlist *funclist, 
                                         struct vty *vty)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
...
static void
opaque_lsa_config_write_if_callback (struct zlist *funclist,
...
static void
opaque_lsa_config_write_debug_callback (struct zlist *funclist, struct vty *vty)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
...
static int
opaque_lsa_originate_callback (struct zlist *funclist, void *lsa_type_dependent)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  int rc = -1;
...
static int
new_lsa_callback (struct zlist *funclist, struct ospf_lsa *lsa)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  int rc = -1;
...
static int
del_lsa_callback (struct zlist *funclist, struct ospf_lsa *lsa)
{
  struct zlistnode *node, *nnode;
  struct ospf_opaque_functab *functab;
  int rc = -1;
      ...
      if (CHECK_FLAG (nbr->options, OSPF_OPTION_O))
        {
          if (! CHECK_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT))
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[DBG] Opaque-LSA: Now get operational!");
...
static void ospf_opaque_lsa_reoriginate_resume (struct zlist *listtop, void *arg);
...
ospf_opaque_lsa_originate_schedule (struct ospf_interface *oi, int *delay0)
{
  ...
  if ((top = oi_to_top (oi)) == NULL || (area = oi->area) == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_lsa_originate_schedule: Invalid argument?");
      goto out;
    }
...
//    }
  ...
  &&    oi->t_opaque_lsa_self == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] Schedule Type-9 Opaque-LSA origination in %d sec later.", delay);
  ...
  &&    area->t_opaque_lsa_self == NULL)
    {
       ...
       */
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] Schedule Type-10 Opaque-LSA origination in %d sec later.", delay);
  ...
  &&    top->t_opaque_lsa_self == NULL)
    {
       ...
       */
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] Schedule Type-11 Opaque-LSA origination in %d sec later.", delay);
...
ospf_opaque_type9_lsa_originate (struct thread *t)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type9-LSA]: Originate Opaque-LSAs for OI %s",
                IF_NAME (oi));
...
ospf_opaque_type10_lsa_originate (struct thread *t)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type10-LSA]: Originate Opaque-LSAs for Area %s",
...
ospf_opaque_type11_lsa_originate (struct thread *t)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type11-LSA]: Originate AS-External Opaque-LSAs");
...
static void
ospf_opaque_lsa_reoriginate_resume (struct zlist *listtop, void *arg)
{
  struct zlistnode *node, *nnode;
  struct opaque_info_per_type *oipt;
  struct ospf_opaque_functab *functab;
  ...
  for (ALL_LIST_ELEMENTS (listtop, node, nnode, oipt))
    {
      ...
      if ((* functab->lsa_originator)(arg) != 0)
        {
          zlog_warn ("[WRN] ospf_opaque_lsa_reoriginate_resume: Failed (opaque-type=%u)", oipt->opaque_type);
          continue;
        }
    }
...
ospf_opaque_lsa_install (struct ospf_lsa *lsa, int rt_recalc)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA_INSTALL))
    zlog_debug ("[DBG] Install Type-%u Opaque-LSA: [opaque-type=%u, opaque-id=%x]", lsa->data->type, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));

  /* Replace the existing lsa with the new one. */
  if ((oipt = lookup_opaque_info_by_type (lsa)) != NULL)
  {
    if ((oipi = lookup_opaque_info_by_id (oipt, lsa)) != NULL)
    {
      ospf_lsa_unlock (&oipi->lsa);
  ...
  if ((oipi == NULL) || (oipt == NULL))
  {
    if ((oipi =  (lsa)) == NULL) /*TODO: Adam is it OK??? */
    {
      zlog_warn ("[WRN]ospf_opaque_lsa_install: register_opaque_lsa() ?");
      goto out;
    }
    zlog_debug ("ospf_opaque_lsa_install oipi = (lsa)"); 
  }
  ...
  switch (lsa->data->type)
      ...
      if ((top = oi_to_top (lsa->oi)) == NULL)
        {
          ...
          zlog_warn ("[WRN] ospf_opaque_lsa_install: Sonmething wrong?");
          goto out;
        }
      break;
    case OSPF_OPAQUE_AREA_LSA:
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_opaque_lsa_install: Unexpected LSA-type(%u)", lsa->data->type);
      goto out;
    }
                                      ...
                                      u_char lsa_type, u_char opaque_type)
{
  ...
  switch (lsa_type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      if ((oi = (struct ospf_interface *) lsa_type_dependent) == NULL)
        {
          zlog_warn ("[WRN] ospf_opaque_lsa_reoriginate_schedule:"
                     " Type-9 Opaque-LSA: Invalid parameter?");
          goto out;
        }
      if ((top = oi_to_top (oi)) == NULL)
        {
          zlog_warn ("[WRN] ospf_opaque_lsa_reoriginate_schedule: OI(%s) -> TOP?",
                     IF_NAME (oi));
          goto out;
        }
      if (!list_isempty (ospf_opaque_type9_funclist)
          ...
          && oi->t_opaque_lsa_self != NULL)
        {
          zlog_warn ("[WRN] Type-9 Opaque-LSA (opaque_type=%u):"
    ...
    case OSPF_OPAQUE_AREA_LSA:
      if ((area = (struct ospf_area *) lsa_type_dependent) == NULL)
        {
          zlog_warn ("[WRN] ospf_opaque_lsa_reoriginate_schedule:"
                     " Type-10 Opaque-LSA: Invalid parameter? area = NULL");
          goto out;
        }
      ...
      if (!list_isempty (ospf_opaque_type10_funclist)
          ...
          && area->t_opaque_lsa_self != NULL)
        {
          zlog_warn ("[WRN] Type-10 Opaque-LSA (opaque_type=%u):"
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      if (!list_isempty (ospf_opaque_type11_funclist)
          ...
          && top->t_opaque_lsa_self != NULL)
        {
          zlog_warn ("[WRN] Type-11 Opaque-LSA (opaque_type=%u):"
  ...
  if (!CHECK_FLAG (top->opaque, OPAQUE_OPERATION_READY_BIT))
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_opaque_lsa_reoriginate_schedule: Not operational.");
      goto out;                 /* This is not an error. */
    }
//  if (IS_OPAQUE_LSA_ORIGINATION_BLOCKED (top->opaque))
//    {
...
//    }
  ...
  if (oipt->t_opaque_lsa_self != NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[WRN] Type-%u Opaque-LSA has already been scheduled to"
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Schedule Type-%u Opaque-LSA to RE-ORIGINATE in %d"
...
ospf_opaque_type9_lsa_reoriginate_timer (struct thread *t)
{
  ...
  ||   functab->lsa_originator == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_type9_lsa_reoriginate_timer: No associated function?");
      goto out;
    }
  ...
  if ((top = oi_to_top (oi)) == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_type9_lsa_reoriginate_timer: Something wrong?");
      goto out;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type9-LSA]: Re-originate Opaque-LSAs (opaque-type=%u) for OI (%s)", oipt->opaque_type, IF_NAME (oi));
...
ospf_opaque_type10_lsa_reoriginate_timer (struct thread *t)
{
  ...
  ||   functab->lsa_originator == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_type10_lsa_reoriginate_timer: No associated function?");
      goto out;
    }
  ...
  if (area == NULL || (top = area->ospf) == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_type10_lsa_reoriginate_timer: Something wrong?");
      goto out;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type10-LSA]: Re-originate Opaque-LSAs"
...
ospf_opaque_type11_lsa_reoriginate_timer (struct thread *t)
{
  ...
  if ((functab = oipt->functab) == NULL
      || functab->lsa_originator == NULL)
    {
      zlog_warn ("[WNR] ospf_opaque_type11_lsa_reoriginate_timer:"
  ...
  if ((top = (struct ospf *) oipt->owner) == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_type11_lsa_reoriginate_timer: Something wrong?");
      goto out;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Type11-LSA]: Re-originate Opaque-LSAs (opaque-type=%u).", oipt->opaque_type);
...
ospf_opaque_lsa_refresh_schedule (struct ospf_lsa *lsa0)
{
  ...
  {
    struct in_addr temp;
    temp.s_addr = lsa0->data->id.s_addr;
    zlog_warn ("[WRN] ospf_opaque_lsa_refresh_schedule: Invalid parameter?: oipt = NULL opaque id %s", inet_ntoa(temp));
    goto out;
  }
  else if ((oipi = lookup_opaque_info_by_id (oipt, lsa0)) == NULL)
  {
    struct in_addr temp;
    temp.s_addr = lsa0->data->id.s_addr;
    zlog_warn ("[WRN] ospf_opaque_lsa_refresh_schedule: Invalid parameter?: oipi = NULL opaque id %s", oipt, oipi, inet_ntoa(temp));
    goto out;
  }
  ...
  if ((lsa = oipi->lsa) == NULL)
    {
      zlog_warn ("[WRN] ospf_opaque_lsa_refresh_schedule: Something wrong?");
      goto out;
    }
  ...
  if (oipi->t_opaque_lsa_self != NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] Type-%u Opaque-LSA has already been scheduled to REFRESH: [opaque-type=%u, opaque-id=%x]", lsa->data->type, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
      goto out;
    }
  ...
  switch (lsa->data->type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_opaque_lsa_refresh_schedule: Unexpected LSA type: %u", lsa->data->type);
      goto out;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Schedule Type-%u Opaque-LSA to REFRESH in %d sec later (opaque-type=%u, opaque-id=%x)", lsa->data->type, delay, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
...
ospf_opaque_lsa_refresh_timer (struct thread *t)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Timer[Opaque-LSA]: (Opaque-LSA Refresh expire)");
...
ospf_opaque_lsa_flush_schedule (struct ospf_lsa *lsa0)
{
  ...
  if ((oipt = lookup_opaque_info_by_type (lsa0)) == NULL)
  {
    struct in_addr temp;
    temp.s_addr = lsa0->data->id.s_addr;
    zlog_warn ("[WRN] ospf_opaque_lsa_flush_schedule: Invalid parameter?: oipt = %x, oipi = %x opaque id %s", oipt, oipi, inet_ntoa(temp));
    goto out;
  }
  else if ((oipi = lookup_opaque_info_by_id (oipt, lsa0)) == NULL)
  {
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: oipi and oipt != NULL");

  /* Given "lsa0" and current "oipi->lsa" may different, but harmless. */
  if ((lsa = oipi->lsa) == NULL)
  {
    zlog_warn ("[WRN] ospf_opaque_lsa_flush_schedule: Something wrong?");
    goto out;
  }
  ...
  {
    struct in_addr tmp;
    tmp.s_addr = lsa0->data->id.s_addr;
    zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: lsa0 id: %s ospf instance %d", inet_ntoa(tmp), lsa0->area->ospf->instance);
    tmp.s_addr = lsa->data->id.s_addr;
    zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: lsa  id: %s ospf instance %d", inet_ntoa(tmp), lsa->area->ospf->instance);
  }
  ...
  switch (lsa->data->type)
    ...
    case OSPF_OPAQUE_AREA_LSA:
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: ospf_ls_retransmit_delete_nbr_area");
      ospf_ls_retransmit_delete_nbr_area (lsa->area, lsa);
      break;
    case OSPF_OPAQUE_AS_LSA:
      ospf_ls_retransmit_delete_nbr_as (lsa0->area->ospf, lsa);
      break;
    default:
      zlog_warn ("[WRN] ospf_opaque_lsa_flush_schedule: Unexpected LSA-type(%u)", lsa->data->type);
      goto out;
    }

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: deleting oipi");
  /* Dequeue listnode entry from the list. */
  ...
  int lsa_lock = lsa->lock;

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug("[DBG] ospf_opaque_lsa_flush_schedule: free_opaque_info_per_id 0x%x", oipi);
  /* Disassociate internal control information with the given lsa. */
  free_opaque_info_per_id ((void *) oipi);

  if (lsa_lock <= 1) //TODO Adam: lsa was removed from memory. That cause signal 11. This code should solve problem
  {
    zlog_err ("[ERR] ospf_opaque_lsa_flush_schedule: LSA was removed from memory. oipi->lsa = NULL");
    goto out;
  }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Schedule Type-%u Opaque-LSA to FLUSH: [opaque-type=%u, opaque-id=%x]", lsa->data->type, GET_OPAQUE_TYPE (ntohl (lsa->data->id.s_addr)), GET_OPAQUE_ID (ntohl (lsa->data->id.s_addr)));
...
out:
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_opaque_lsa_flush_schedule OK");
  return;
}
...
void
ospf_opaque_adjust_lsreq (struct ospf_neighbor *nbr, struct zlist *lsas)
{
  struct ospf *top;
  struct ospf_area *area;
  struct ospf_interface *oi;
  struct zlistnode *node1, *nnode1;
  struct zlistnode *node2, *nnode2;
  struct ospf_lsa *lsa;
                                          ...
                                          struct ospf_lsa *lsa)
{
  ...
  switch (lsa->data->type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_opaque_self_originated_lsa_received: Unexpected LSA-type(%u)", lsa->data->type);
      return;
    }

//zlog_debug("ospf_opaque_self_originated_lsa_received: lsa discard");
...
ospf_opaque_ls_ack_received (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  ...
  switch (lsa->data->type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_opaque_ls_ack_received: Unexpected LSA-type(%u)", lsa->data->type);
      return;
    }
...
oi_to_top (struct ospf_interface *oi)
{
  ...
  if (oi == NULL || (area = oi->area) == NULL || (top = area->ospf) == NULL)
    zlog_warn ("[WRN] Broken relationship for \"OI -> AREA -> OSPF\"?");
