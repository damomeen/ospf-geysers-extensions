		      ...
		      struct ospf_area *area)
{
  ...
  if (rn->info)
    {
      zlog_warn ("[WRN] Same routing information exists for %s", inet_ntoa (v->id));
      route_unlock_node (rn);
      return;
    }
		       ...
		       struct ospf_area *area)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_router: Start");

  lsa = (struct router_lsa *) v->lsa;

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_router: LS ID: %s",
  ...
  if (! IS_ROUTER_LSA_BORDER (lsa) && ! IS_ROUTER_LSA_EXTERNAL (lsa))
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_intra_add_router: "
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_router: talking about %s/%d",
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_router: Stop");
}
		     ...
		     struct vertex *v, struct ospf_area *area)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_stub(): Start");
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_stub(): processing route to %s/%d",  
	       inet_ntoa (p.prefix), p.prefixlen);
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_stub(): calculated cost is %d + %d = %d", 
  ...
  if (rn->info)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_intra_add_stub(): "
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_intra_add_stub(): old route is better, exit");
	  return;
	}
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_intra_add_stub(): routes are equal, merge");
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_intra_add_stub(): new route is better, set it");
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_intra_add_stub(): installing new route");
  ...
  if (v != area->spf)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_intra_add_stub(): this network is on remote router");
      ospf_route_copy_nexthops_from_vertex (or, v);
    }
  else
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_intra_add_stub(): this network is on this router");

      if ((oi = ospf_if_lookup_by_prefix (area->ospf, &p)))
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_intra_add_stub(): the interface is %s",
		       IF_NAME (oi));
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_intra_add_stub(): where's the interface ?");
	}
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug("[DBG] ospf_intra_add_stub(): Stop");
}
...
ospf_route_table_dump (struct route_table *rt)
{
  ...
  zlog_debug ("[DBG] ========== OSPF routing table ==========");
  for (rn = route_top (rt); rn; rn = route_next (rn))
    if ((or = rn->info) != NULL)
      {
        if (or->type == OSPF_DESTINATION_NETWORK)
...
static int
ospf_path_exist (struct zlist *plist, struct in_addr nexthop,
		 struct ospf_interface *oi)
{
  struct zlistnode *node, *nnode;
  struct ospf_path *path;
...
struct ospf_path *
ospf_path_lookup (struct zlist *plist, struct ospf_path *path)
{
  struct zlistnode *node;
  struct ospf_path *op;
...
void
ospf_route_copy_nexthops (struct ospf_route *to, struct zlist *from)
{
  struct zlistnode *node, *nnode;
  struct ospf_path *path;
...
void
ospf_route_subst_nexthops (struct ospf_route *to, struct zlist *from)
{
...
ospf_prune_unreachable_networks (struct route_table *rt)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Pruning unreachable networks");
...
ospf_prune_unreachable_routers (struct route_table *rtrs)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Pruning unreachable routers");
