...
ospf_dr_election_sub (struct zlist *routers)
{
...
static struct ospf_neighbor *
ospf_elect_dr (struct ospf_interface *oi, struct zlist *el_list)
{
  struct zlist *dr_list;
  struct zlistnode *node;
...
static struct ospf_neighbor *
ospf_elect_bdr (struct ospf_interface *oi, struct zlist *el_list)
{
  struct zlist *bdr_list, *no_dr_list;
  struct zlistnode *node;
...
static void
ospf_dr_eligible_routers (struct route_table *nbrs, struct zlist *el_list)
{
...
ospf_dr_election (struct ospf_interface *oi)
{
  ...
  zlog_info ("[INF] DR-Election[1st]: Backup %s", inet_ntoa (BDR (oi)));
  zlog_info ("[INF] DR-Election[1st]: DR     %s", inet_ntoa (DR (oi)));
  ...
  if (new_state != old_state &&
      !(new_state == ISM_DROther && old_state < ISM_DROther))
    {
      ...
      zlog_info ("[INF] DR-Election[2nd]: Backup %s", inet_ntoa (BDR (oi)));
      zlog_info ("[INF] DR-Election[2nd]: DR     %s", inet_ntoa (DR (oi)));
    }
