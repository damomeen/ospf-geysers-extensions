...
msg_print (struct msg *msg)
{
  ...
  zlog_debug
    ("API-msg [%s]: type(%d),len(%d),seq(%lu),data(%p),size(%u)",
...
msg_read (int fd)
{
  ...
  if (rlen < 0)
    {
      zlog_warn ("[WRN] msg_read: readn %s", safe_strerror (errno));
      return NULL;
    }
  else if (rlen == 0)
    {
      zlog_warn ("[WRN] msg_read: Connection closed by peer");
      return NULL;
    }
  else if (rlen != sizeof (struct apimsghdr))
    {
      zlog_warn ("[WRN] msg_read: Cannot read message header!");
      return NULL;
    }
  ...
  if (hdr.version != OSPF_API_VERSION)
    {
      zlog_warn ("[WRN] msg_read: OSPF API protocol version mismatch");
      return NULL;
    }
  ...
  if (bodylen > 0)
	...
	{
	  zlog_warn ("[WRN] msg_read: Cannot read message body!");
	  return NULL;
	}
    }
...
msg_write (int fd, struct msg *msg)
{
  ...
  if (wlen < 0)
    {
      zlog_warn ("[WRN] msg_write: writen %s", safe_strerror (errno));
      return -1;
    }
  else if (wlen == 0)
    {
      zlog_warn ("[WRN] msg_write: Connection closed by peer");
      return -1;
    }
  else if (wlen != l)
    {
      zlog_warn ("[WRN] msg_write: Cannot write API message");
      return -1;
    }
  return 0;
}
