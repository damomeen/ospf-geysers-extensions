		       ...
		       struct ospf_route *abr_or)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ia_network_route(): processing summary route to %s/%d", 
  ...
  if ((rn1 = route_node_lookup (rt, (struct prefix *) p)))
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_network_route(): "
    ...
    { /* no route */
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_ia_network_route(): add new route to %s/%d",
		   inet_ntoa (p->prefix), p->prefixlen);
                      ...
                      struct ospf_route *new_or, struct ospf_route *abr_or)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ia_router_route(): considering %s/%d", 
	       inet_ntoa (p->prefix), p->prefixlen);
  /* Find a route to the same dest */
  ...
  if (or)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_ia_router_route(): "
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_router_route(): merging the new route");
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_router_route(): skipping the new route");
	  ospf_route_free (new_or);
	  return;
	}
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ia_router_route(): adding the new route"); 
		     ...
		     struct route_table *rtrs, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] process_summary_lsa(): LS ID: %s", inet_ntoa (sl->header.id));
                           ...
                           struct ospf_area *area)
{
  ...
  if (abr_or == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_update_network_route(): can't find a route to the ABR");
      return;
    }
  ...
  if (! rn)
    {
      if (ospf->abr_type != OSPF_ABR_SHORTCUT)
        return; /* Standard ABR can update only already installed
                   backbone paths                                       */
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_update_network_route(): "
  ...
  if (or->path_type != OSPF_PATH_INTRA_AREA &&
    ...
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_update_network_route(): ERR: path type is wrong");
      return;
    }
  ...
  if (ospf->abr_type == OSPF_ABR_SHORTCUT)
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_update_network_route(): Shortcut: "
  ...
  if (or->cost < cost)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_update_network_route(): new route is worse");
      return;
    }
                          ...
                          struct ospf_area *area)
{
  ...
  if (abr_or == NULL)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_update_router_route(): can't find a route to the ABR");
      return;
    }
  ...
  if (!(or->u.std.flags & ROUTER_LSA_EXTERNAL))
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_upd_router_route(): the remote router is not an ASBR");
      return;
    }
			     ...
			     struct route_table *rtrs, struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] process_transit_summaries(): LS ID: %s",
  ...
  if (metric == OSPF_LS_INFINITY)
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] process_transit_summaries(): metric is infinity, skip");
      return 0;
    }
  ...
  if (IS_LSA_MAXAGE (lsa))
    {
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] process_transit_summaries(): This LSA is too old");
      return 0;
    }
  ...
  if (ospf_lsa_is_self_originated (area->ospf, lsa))
    { 
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] process_transit_summaries(): This LSA is mine, skip");
      return 0;
    }
                 ...
                 struct route_table *rtrs)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ia_routing():start");
  ...
  if (IS_OSPF_ABR (ospf))
    {
      ...
      switch (ospf->abr_type)
        ...
        case OSPF_ABR_STAND:
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_routing():Standard ABR");
	      ...
	      if (IS_DEBUG_OSPF_EVENT)
		{
		  zlog_debug ("[DBG] ospf_ia_routing():backbone area found");
		  zlog_debug ("[DBG] ospf_ia_routing():examining summaries");
		}
	    ...
	    if (IS_DEBUG_OSPF_EVENT)
	      zlog_debug ("[DBG] ospf_ia_routing():backbone area NOT found");
          break;
        case OSPF_ABR_IBM:
        case OSPF_ABR_CISCO:
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_routing():Alternative Cisco/IBM ABR");
		...
		{
		  zlog_debug ("[DBG] ospf_ia_routing(): backbone area found");
		  zlog_debug ("[DBG] ospf_ia_routing(): examining BB summaries");
		}
	      ...
	      if (IS_DEBUG_OSPF_EVENT)
		zlog_debug ("[DBG] ospf_ia_routing(): "
	  ...
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_ia_routing():Alternative Shortcut");
          area = ospf->backbone; /* Find the BB */
  ...
  else 
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_ia_routing():not ABR, considering all areas");
