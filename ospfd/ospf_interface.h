...
#define OSPF_IF_PASSIVE_STATUS(O) \
  ...
  DECLARE_IF_PARAM (struct zlist *, auth_crypt);   /* List of Auth cryptographic data. */
...
#define PRIORITY(I)		((I)->nbr_self->priority)
  ...
  struct zlist *nbr_nbma;

  /* self-originated LSAs. */
  struct ospf_lsa *network_lsa_self;	/** network-LSA. */
#ifdef HAVE_OPAQUE_LSA
  struct zlist *opaque_lsa_self;        /** Type-9 Opaque-LSAs */
#endif /* HAVE_OPAQUE_LSA */

  struct route_table *ls_upd_queue;

  struct zlist *ls_ack;                 /** Link State Acknowledgment list. */
...
extern struct ospf_interface *ospf_if_lookup_recv_if (struct ospf *, struct in_addr);
...
extern struct crypt_key *ospf_crypt_key_lookup (struct zlist *, u_char);
extern struct crypt_key *ospf_crypt_key_new (void);
extern void ospf_crypt_key_add (struct zlist *, struct crypt_key *);
extern int ospf_crypt_key_delete (struct zlist *, u_char);
...
extern int conf_ospf_hello_interval;

extern int createRC (struct in_addr RCaddress, int addr_len);
