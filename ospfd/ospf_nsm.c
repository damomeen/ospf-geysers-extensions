...
ospf_inactivity_timer (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_TIMERS))
    zlog (NULL, LOG_DEBUG, "[DBG] NSM[%s:%s]: Timer (Inactivity timer expire)",
...
ospf_db_desc_timer (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_TIMERS))
    zlog (NULL, LOG_DEBUG, "[DBG] NSM[%s:%s]: Timer (DD Retransmit timer expire)",
...
nsm_kill_nbr (struct ospf_neighbor *nbr)
{
  ...
  if (nbr->oi->type == OSPF_IFTYPE_NBMA && nbr->nbr_nbma != NULL)
	...
	zlog_debug ("[DBG] NSM[%s:%s]: Down (PollIntervalTimer scheduled)",
...
nsm_notice_state_change (struct ospf_neighbor *nbr, int next_state, int event)
{
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_STATUS))
    zlog_debug ("[DBG] NSM[%s:%s]: State change %s -> %s (%s)",
               IF_NAME (nbr->oi), inet_ntoa (nbr->router_id),
...
nsm_change_state (struct ospf_neighbor *nbr, int state)
{
  ...
  if ((old_state != NSM_Full && state == NSM_Full) ||
      ...
      else
	{
      ...
      zlog_info ("[INF] NSM_STATE_CHANGE: %s (%s -> %s): "
...
#else
      OSPF_ISM_EVENT_EXECUTE_NO_LOCK (oi, ISM_NeighborChange);
#endif
    break;
  default:
...
ospf_nsm_event (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF (nsm, NSM_EVENTS))
    zlog_debug ("[DBG] NSM[%s:%s]: %s (%s)", IF_NAME (nbr->oi),
	       inet_ntoa (nbr->router_id),
	       LOOKUP (ospf_nsm_state_msg, nbr->state),
	       ospf_nsm_event_str [event]);
