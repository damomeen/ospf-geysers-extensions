 ...
 */
static struct zlist vertex_list = { .del = ospf_vertex_free };

...
ospf_vertex_new (struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] %s: Created %s vertex %s distance: %d", __func__,
...
ospf_vertex_free (void *data)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] %s: Free %s vertex %s", __func__,
		 ...
		 int print_parents, int print_children)
{
  ...
  zlog_debug("[DBG] %s %s vertex %s  distance %u flags %u",
            msg,
	    v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
	    inet_ntoa(v->lsa->id),
	    v->distance,
  ...
  if (print_parents)
    {
	  ...
	  if (vp)
	    {
	      zlog_debug ("[DBG] parent %s backlink %d nexthop %s  interface %s",
...
/*    assert (distance <= w->distance); */
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      ...
      zlog_debug ("[DBG] %s: Adding %s as parent of %s",
                __func__,
  ...
  if (distance < w->distance)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] %s: distance %d better than %d, flushing existing parents",
                          ...
                          unsigned int distance)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("[DBG] ospf_nexthop_calculation(): Start");
      ospf_vertex_dump("V (parent):", v, 1, 1);
      ospf_vertex_dump("W (dest)  :", w, 1, 1);
      zlog_debug ("[DBG] V->W distance: %d", distance);
    }
          ...
          if (IS_DEBUG_OSPF_EVENT)
            {
              ...
              zlog_debug("[DBG] ospf_nexthop_calculation(): considering link "
          ...
          if (l->m[0].type == LSA_LINK_TYPE_POINTOPOINT)
            {
              ...
              */
              ...
              if (oi && l2)
                ...
                zlog_info("[INF] ospf_nexthop_calculation(): "
   ...
   */
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] %s: Intervening routers, adding parent(s)", __func__);
	       ...
	       struct pqueue * candidate)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] %s: Next vertex of %s vertex %s",
                __func__, 
                v->type == OSPF_VERTEX_ROUTER ? "Router" : "Network",
                inet_ntoa(v->lsa->id));
  ...
  while (p < lim)
    {
      ...
      if (v->lsa->type == OSPF_ROUTER_LSA)
          ...
          switch (type)
            ...
            case LSA_LINK_TYPE_VIRTUALLINK:
              if (type == LSA_LINK_TYPE_VIRTUALLINK)
                {
                  if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug ("[DBG] looking up LSA through VL: %s",
                               inet_ntoa (l->link_id));
                }
              ...
              if (w_lsa)
                {
                  if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug ("[DBG] found Router LSA %s", inet_ntoa (l->link_id));
                }
              break;
            case LSA_LINK_TYPE_TRANSIT:
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[DBG] Looking up Network LSA, ID: %s",
                           inet_ntoa (l->link_id));
              w_lsa = ospf_lsa_lookup_by_id (area, OSPF_NETWORK_LSA,
                                             l->link_id);
              if (w_lsa)
                if (IS_DEBUG_OSPF_EVENT)
                  zlog_debug ("[DBG] found the LSA");
              break;
            default:
              zlog_warn ("[WRN] Invalid LSA link type %d", type);
              continue;
            }
        }
      else
        {
          ...
          if (w_lsa)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[DBG] found Router LSA %s", inet_ntoa (w_lsa->data->id));
            }
        }
      ...
      if (ospf_lsa_has_link (w_lsa->data, v->lsa) < 0 )
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("[DBG] The LSA doesn't have a link back");
          continue;
        }
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] The LSA is already in SPF");
	  continue;
	}
      ...
      if (w_lsa->stat == LSA_SPF_NOT_EXPLORED)
	{
          ...
          if (ospf_nexthop_calculation (area, v, w, l, distance))
            ...
            zlog_debug ("[DBG] Nexthop Calc failed");
	}
...
ospf_spf_dump (struct vertex *v, int i)
{
  ...
  if (v->type == OSPF_VERTEX_ROUTER)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] SPF Result: %d [R] %s", i, inet_ntoa (v->lsa->id));
    }
  else
    {
      struct network_lsa *lsa = (struct network_lsa *) v->lsa;
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] SPF Result: %d [N] %s/%d", i, inet_ntoa (v->lsa->id),
                   ip_masklen (lsa->mask));
    }
                        ...
                        struct route_table *rt)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_process_stub():processing stubs for area %s",
  ...
  if (v->type == OSPF_VERTEX_ROUTER)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_process_stubs():processing router LSA, id: %s",
                   inet_ntoa (v->lsa->id));
      rlsa = (struct router_lsa *) v->lsa;


      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_process_stubs(): we have %d links to process",
                   ntohs (rlsa->links));
      p = ((u_char *) v->lsa) + OSPF_LSA_HEADER_SIZE + 4;
      lim = ((u_char *) v->lsa) + ntohs (v->lsa->length);
...
ospf_rtrs_free (struct route_table *rtrs)
{
  ...
  struct zlist *or_list;
  struct ospf_route *or;
  struct zlistnode *node, *nnode;

  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Route: Router Routing Table free");
...
ospf_rtrs_print (struct route_table *rtrs)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_rtrs_print() start");
          ...
          for (ALL_LIST_ELEMENTS_RO (or->paths, pnode, path))
            {
              if (path->nexthop.s_addr == 0)
                {
                  if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug ("[DBG]    directly attached to %s\r\n",
                               IF_NAME (path->oi));
                }
              else
                {
                  if (IS_DEBUG_OSPF_EVENT)
                    zlog_debug ("[DBG]    via %s, %s\r\n",
                               inet_ntoa (path->nexthop), IF_NAME (path->oi));
                }
            }
        }

  zlog_debug ("[DBG] ospf_rtrs_print() end");
}
                    ...
                    struct route_table *new_rtrs)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("[DBG] ospf_spf_calculate: Start");
      zlog_debug ("[DBG] ospf_spf_calculate: running Dijkstra for area %s",
                 inet_ntoa (area->area_id));
    }
  ...
  if (!area->router_lsa_self)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_spf_calculate: "
  ...
  *(v->stat) = LSA_SPF_IN_SPFTREE;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_spf_calculate: Stop. %ld vertices",
...
ospf_spf_calculate_timer (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] SPF: Timer (SPF calculation expire)");
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] SPF: calculation complete");
...
ospf_spf_calculate_schedule (struct ospf *ospf)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] SPF: calculation timer scheduled");
  ...
  if (ospf->t_spf_calc)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] SPF: calculation timer is already scheduled: %p",
                   ospf->t_spf_calc);
      return;
    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] SPF: calculation timer delay = %ld", delay);
