...
ospf_apiserver_if_lookup_by_ifp (struct interface *ifp)
{
  ...
  {
    case INNI:
      if (!(ospf = ospf_inni_lookup()))
      {
        zlog_warn("[WRN] ospf_apiserver_if_lookup_by_ifp: can't find INNI instance");
        return NULL;
      }
      break;
    case ENNI:
      if (!(ospf = ospf_enni_lookup()))
      {
        zlog_warn("[WRN] ospf_apiserver_if_lookup_by_ifp: can't find ENNI instance");
        return NULL;
      }
      break;
    case UNI:
      if (!(ospf = ospf_uni_lookup()))
      {
        zlog_warn("[WRN] ospf_apiserver_if_lookup_by_ifp: can't find UNI instance");
        return NULL;
      }
      break;
    default:
...
ospf_apiserver_init (void)
{
  ...
  if (rc != 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_init: Failed to register opaque type [0/0]");
    }
...
ospf_apiserver_read (struct thread *thread)
{
  ...
  if (fd == apiserv->fd_sync)
    ...
    {
      zlog_warn ("[WRN] ospf_apiserver_read: Unknown fd(%d)", fd);
      ospf_apiserver_free (apiserv);
      goto out;
    }
  ...
  if (msg == NULL)
    {
      zlog_warn("[WRN] ospf_apiserver_read: read failed on fd=%d, closing connection", fd);
...
ospf_apiserver_sync_write (struct thread *thread)
{
  ...
  if (fd != apiserv->fd_sync)
    {
      zlog_warn ("[WRN] ospf_apiserver_sync_write: Unknown fd=%d", fd);
      goto out;
    }
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] API: ospf_apiserver_sync_write: No message in Sync-FIFO?");
      return 0;
    }
  ...
  if (rc < 0)
    {
      zlog_warn("[WRN] ospf_apiserver_sync_write: write failed on fd=%d", fd);
      goto out;
    }
...
ospf_apiserver_async_write (struct thread *thread)
{
  ...
  if (fd != apiserv->fd_async)
    {
      zlog_warn ("[WRN] ospf_apiserver_async_write: Unknown fd=%d", fd);
      goto out;
    }
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] API: ospf_apiserver_async_write: No message in Async-FIFO?");
      return 0;
    }
  ...
  if (rc < 0)
    {
      zlog_warn("[WRN] ospf_apiserver_async_write: write failed on fd=%d", fd);
      goto out;
    }
...
ospf_apiserver_serv_sock_family (unsigned short port, int family)
{
  ...
  if (rc < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_serv_sock_family: listen: %s",
                 safe_strerror (errno));
      close (accept_sock);	/* Close socket */
      return rc;
    }
...
ospf_apiserver_accept (struct thread *thread)
{
  ...
  if (new_sync_sock < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_accept: accept: %s", safe_strerror (errno));
      return -1;
    }
  ...
  if (ret < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_accept: getpeername: %s", safe_strerror (errno));
      close (new_sync_sock);
      return -1;
    }
  ...
  if (ntohs (peer_async.sin_port) == ospf_apiserver_getport ())
    {
      zlog_warn ("[WRN] API: ospf_apiserver_accept: Peer(%s/%u): Invalid async port number?",
  ...
  if (new_async_sock < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_accept: socket: %s", safe_strerror (errno));
      close (new_sync_sock);
      return -1;
    }
  ...
  if (ret < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_accept: connect: %s", safe_strerror (errno));
      close (new_sync_sock);
  ...
  if (ret < 0)
    {
      zlog_warn ("[WRN] ospf_apiserver_accept: shutdown: %s", safe_strerror (errno));
      close (new_sync_sock);
      close (new_async_sock);
      return -1;
    }
...
ospf_apiserver_send_msg (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_NSM_CHANGE:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_apiserver_send_msg: Unknown message type %d",
		 msg->hdr.msgtype);
      return -1;
    }
			   ...
			   u_char rc)
{
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] ospf_apiserver_send_reply: msg_new failed");
#ifdef NOTYET
...
ospf_apiserver_handle_msg (struct ospf_apiserver *apiserv, struct msg *msg)
{
  ...
  switch (msg->hdr.msgtype)
    ...
    case MSG_DELETE_REQUEST:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_apiserver_handle_msg: Unknown message type: %d",
		 msg->hdr.msgtype);
      rc = -1;
    }
  return rc;
				     ...
				     u_char lsa_type, u_char opaque_type)
{
  ...
  switch (lsa_type)
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn ("[WRN] ospf_apiserver_register_opaque_type: lsa_type(%d)",
		 lsa_type);
  ...
  if (rc != 0)
    {
      zlog_warn ("[WRN] Failed to register opaque type [%d/%d]",
				       ...
				       u_char lsa_type, u_char opaque_type)
{
  ...
  zlog_warn ("[WRN] Failed to unregister opaque type [%d/%d]",
	     lsa_type, opaque_type);
	  ...
	  if (r->lsa_type == OSPF_OPAQUE_LINK_LSA)
	    {
	      ...
	      if (!msg)
		{
		  zlog_warn ("[WRN] apiserver_notify_ready_type9: msg_new failed");
#ifdef NOTYET
		  /* Cannot allocate new message. What should we do? */
		  ospf_apiserver_free (apiserv);
#endif
      ...
      for (ALL_LIST_ELEMENTS (apiserv->opaque_types, node2, nnode2, r))
	{
	  ...
	  if (r->lsa_type == OSPF_OPAQUE_AREA_LSA)
		...
		new_msg_ready_notify (0, OSPF_OPAQUE_AREA_LSA,
	      ...
	      if (!msg)
		{
		  zlog_warn ("[WRN] apiserver_notify_ready_type10: msg_new failed");
#ifdef NOTYET
		  /* Cannot allocate new message. What should we do? */
		  ospf_apiserver_free (apiserv);
#endif
...
ospf_apiserver_notify_ready_type11 (struct ospf_apiserver *apiserv)
{
  ...
  for (ALL_LIST_ELEMENTS (apiserv->opaque_types, node, nnode, r))
      ...
      if (r->lsa_type == OSPF_OPAQUE_AS_LSA)
	{
	  ...
	  if (!msg)
	    {
	      zlog_warn ("[WRN] apiserver_notify_ready_type11: msg_new failed");
#ifdef NOTYET
	      /* Cannot allocate new message. What should we do? */
	      ospf_apiserver_free (apiserv);
#endif
...
apiserver_sync_callback (struct ospf_lsa *lsa, void *p_arg, int int_arg)
{
  ...
  if ((param->filter->origin == ANY_ORIGIN) ||
      ...
      if (lsa->data->type == OSPF_OPAQUE_LINK_LSA)
	{
      ...
      if (!msg)
	{
	  zlog_warn ("[WRN] apiserver_sync_callback: new_msg_update failed");
#ifdef NOTYET
	  /* Cannot allocate new message. What should we do? */
			       ...
			       struct lsa_header *protolsa)
{
  ...
  if ((s = stream_new (OSPF_MAX_LSA_SIZE)) == NULL)
    {
      zlog_warn ("[WRN] ospf_apiserver_opaque_lsa_new: stream_new failed");
      return NULL;
    }
  ...
  if ((new = ospf_lsa_new ()) == NULL)
    {
      zlog_warn ("[WRN] ospf_apiserver_opaque_lsa_new: ospf_lsa_new() ?");
      stream_free (s);
      return NULL;
    }
  ...
  if ((new->data = ospf_lsa_data_new (length)) == NULL)
    {
      zlog_warn ("[WRN] ospf_apiserver_opaque_lsa_new: ospf_lsa_data_new() ?");
      ospf_lsa_unlock (&new);
      stream_free (s);
      return NULL;
    }
					 ...
					 struct msg *msg)
{
  ...
  switch (data->type)
    ...
    case OSPF_OPAQUE_LINK_LSA:
      ...
      if (!oi)
	{
	  zlog_warn ("[WRN] apiserver_originate: unknown interface %s",
    ...
    case OSPF_OPAQUE_AREA_LSA:
      ...
      if (!area)
	{
	  zlog_warn ("[WRN] apiserver_originate: unknown area %s",
    ...
    default:
      ...
      zlog_warn ("[WRN] apiserver_originate: Cannot originate non-opaque LSA type %d",
		 data->type);
  ...
  if (!apiserver_is_opaque_type_registered (apiserv, lsa_type, opaque_type))
    {
      zlog_warn ("[WRN] apiserver_originate: LSA-type(%d)/Opaque-type(%d): Not registered", lsa_type, opaque_type);
  ...
  if (!ready)
    {
      zlog_warn ("[WRN] Neighbors not ready to originate type %d", data->type);
...
ospf_apiserver_originate1 (struct ospf_lsa *lsa)
{
  ...
  if (ospf_lsa_install (ospf, lsa->oi, lsa) == NULL)
    {
      zlog_warn ("[WRN] ospf_apiserver_originate1: ospf_lsa_install failed");
      return -1;
    }
...
ospf_apiserver_lsa_refresher (struct ospf_lsa *lsa)
{
  ...
  if (!apiserv)
    {
      zlog_warn ("[WRN] ospf_apiserver_lsa_refresher: LSA[%s]: No apiserver?", dump_lsa_key (lsa));
  ...
  if (!new)
    {
      ...
      if (!new)
        {
          zlog_warn ("[WRN] ospf_apiserver_lsa_refresher: Cannot create a new LSA?");
          goto out;
        }
    }
  else
  ...
  if (ospf_lsa_install (ospf, new->oi, new) == NULL)
    {
      zlog_warn ("[WRN] ospf_apiserver_lsa_refresher: ospf_lsa_install failed");
      ospf_lsa_unlock (&new);
      goto out;
    }
				      ...
				      struct msg *msg)
{
  ...
  switch (dmsg->lsa_type)
    ...
    case OSPF_OPAQUE_AREA_LSA:
      ...
      if (!area)
	{
	  zlog_warn ("[WRN] ospf_apiserver_lsa_delete: unknown area %s",
    ...
    case OSPF_OPAQUE_AS_LSA:
      ...
      break;
    default:
      zlog_warn("[WRN] ospf_apiserver_lsa_delete: Cannot delete non-opaque LSA type %d",
	 dmsg->lsa_type);
      rc = OSPF_API_ILLEGALLSATYPE;
      goto out;
    }
  ...
  if (!apiserver_is_opaque_type_registered (apiserv, lsa_type, opaque_type))
    {
      zlog_warn ("[WRN] ospf_apiserver_lsa_delete: LSA-type(%d)/Opaque-type(%d): Not registered", lsa_type, opaque_type);
      rc = OSPF_API_OPAQUETYPENOTREGISTERED;
      goto out;
    }
  ...
  if (!old)
    {
      zlog_warn ("[WRN] ospf_apiserver_lsa_delete: LSA[Type%d:%s] not in LSDB",
...
ospf_apiserver_new_if (struct interface *ifp)
{
  ...
  if (ifp->name[0] == '\0') {
    ...
    zlog_warn ("[WRN] OSPF_APISERVER_NEW_IF: Interface has no name?");
    return 0;
  }

  zlog_debug ("[DBG] OSPF_APISERVER_NEW_IF: ifp name=%s, status=%d, index=%d",
  ...
  if (!oi) {
    ...
    zlog_warn ("[WRN] OSPF_APISERVER_NEW_IF: Interface %s not known to OSPFd?", 
	       ifp->name);
    return 0;
  }
...
ospf_apiserver_del_if (struct interface *ifp)
{
  ...
  zlog_warn ("[DBG] OSPF_APISERVER_DEL_IF");
  zlog_warn ("[DBG]   ifp name=%s, status=%d, index=%d\n", ifp->name, ifp->status,
	     ifp->ifindex);
...
ospf_apiserver_ism_change (struct ospf_interface *oi, int old_state)
{
  ...
  zlog_debug ("[DBG] OSPF_APISERVER_ISM_CHANGE");
  if (listcount (apiserver_list) > 0) {
    ospf_apiserver_clients_notify_ism_change (oi);
  }

  zlog_debug ("[DBG]   oi->ifp->name=%s", oi->ifp->name);
  zlog_debug ("[DBG]   old_state=%d", old_state);
  zlog_debug ("[DBG]   oi->state=%d", oi->state);
}
...
ospf_apiserver_nsm_change (struct ospf_neighbor *nbr, int old_status)
{
  ...
  zlog_debug ("[DEB] OSPF_APISERVER_NSM_CHANGE");
  if (listcount (apiserver_list) > 0) {
...
ospf_apiserver_clients_notify_ready_type9 (struct ospf_interface *oi)
{
  ...
  if (!oi->address)
    {
      zlog_warn ("[WRN] Interface has no address?");
      return;
    }

  if (!ospf_apiserver_is_ready_type9 (oi))
    {
      zlog_warn ("[WRN] Interface not ready for type 9?");
      return;
    }
	  ...
	  if (r->lsa_type == OSPF_OPAQUE_LINK_LSA)
	    {
	      ...
	      if (!msg)
		{
		  zlog_warn("[WRN] ospf_apiserver_clients_notify_ready_type9: new_msg_ready_notify failed");
#ifdef NOTYET
		  /* Cannot allocate new message. What should we do? */
		  ospf_apiserver_free (apiserv);
#endif
...
ospf_apiserver_clients_notify_ready_type10 (struct ospf_area *area)
{
  ...
  if (!ospf_apiserver_is_ready_type10 (area))
    {
      zlog_warn ("[WRN] Area not ready for type 10?");
      return;
    }
	  ...
	  if (r->lsa_type == OSPF_OPAQUE_AREA_LSA)
	    {
	      ...
	      if (!msg)
		{
		  zlog_warn("[WRN] ospf_apiserver_clients_notify_ready_type10: new_msg_ready_nofity failed");
#ifdef NOTYET
		  /* Cannot allocate new message. What should we do? */
		  ospf_apiserver_free (apiserv);
#endif
...
ospf_apiserver_clients_notify_ready_type11 (struct ospf *top)
{
  ...
  if (!ospf_apiserver_is_ready_type11 (top))
    {
      zlog_warn ("[WRN] AS not ready for type 11?");
      return;
    }
	  ...
	  if (r->lsa_type == OSPF_OPAQUE_AS_LSA)
	    {
	      ...
	      if (!msg)
		{
		  zlog_warn("[WRN] ospf_apiserver_clients_notify_ready_type11: new_msg_ready_notify failed");
#ifdef NOTYET
		  /* Cannot allocate new message. What should we do? */
		  ospf_apiserver_free (apiserv);
#endif
...
ospf_apiserver_clients_notify_ism_change (struct ospf_interface *oi)
{
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] apiserver_clients_notify_ism_change: msg_new failed");
      return;
    }
...
ospf_apiserver_clients_notify_nsm_change (struct ospf_neighbor *nbr)
{
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] apiserver_clients_notify_nsm_change: msg_new failed");
      return;
    }
...
apiserver_clients_lsa_change_notify (u_char msgtype, struct ospf_lsa *lsa)
{
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] apiserver_clients_lsa_change_notify: msg_new failed");
      return;
    }
...
apiserver_notify_clients_lsa (u_char msgtype, struct ospf_lsa *lsa)
{
  ...
  if (!msg)
    {
      zlog_warn ("[WRN] notify_clients_lsa: msg_new failed");
      return -1;
    }
