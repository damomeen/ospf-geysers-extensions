...
ospf_router_id_update (struct ospf *ospf)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Router-ID[OLD:%s]: Update", inet_ntoa (ospf->router_id));
  ...
  */
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Router-ID[NEW:%s]: Update", inet_ntoa (ospf->router_id));
...
static struct ospf *
ospf_new (adj_type_t interface_type)
{
  int i;
  ...
  new->instance = interface_type;

  if ((new->fd = ospf_sock_init(new)) < 0)
    {
      zlog_err("[ERR] ospf_new: fatal error: ospf_sock_init was unable to open "
	       "a socket");
      exit(1);
    }
  if ((new->ibuf = stream_new(OSPF_MAX_PACKET_SIZE+1)) == NULL)
    {
      zlog_err("[ERR] ospf_new: fatal error: stream_new(%u) failed allocating ibuf",
	       OSPF_MAX_PACKET_SIZE+1);
      exit(1);
    }
  new->t_read = thread_add_read (master, ospf_read, new, new->fd);
  new->oi_write_q = list_new ();
  new->rc_interface = NULL;
...
ospf_inni_lookup()
{
  ...
  struct zlistnode *node;
  struct ospf *ospf_rez;

  for (ALL_LIST_ELEMENTS_RO (om->ospf, node, ospf_rez))
    {
      if (ospf_rez->instance == INNI)
	return ospf_rez;
    }
    return NULL;
}
...
/*
 this function is used to bring ospf up on all the interfaces 
 that are connected to the network specified by prefix.
...
ospf_network_run (struct ospf *ospf, struct prefix *p, struct ospf_area *area)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (om->iflist, node, ifp))
	...
	continue;
#ifdef GMPLS
      if (ifp->ospf_instance != ospf -> instance)
	continue;
#endif /* GMPLS */
...
#else
      OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr_nbma->nbr, NSM_KillNbr);
#endif /* HAVE_OMNIORB */
    }
...
#else
      OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_Start);
#endif /* HAVE_OMNIORB */
    }
}
