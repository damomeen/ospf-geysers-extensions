...
ospf_packet_add (struct ospf_interface *oi, struct ospf_packet *op)
{
  if (!oi->obuf)
    {
      zlog_err("[ERR] ospf_packet_add(interface %s in state %d [%s], packet type %s, "
...
ospf_packet_dup (struct ospf_packet *op)
{
  ...
  if (stream_get_endp(op->s) != op->length)
    /* XXX size_t */
    zlog_warn ("[WRN] ospf_packet_dup stream %lu ospf_packet %u size mismatch",
                       ...
                       u_int16_t length)
{
  ...
  if (ck == NULL)
    {
      zlog_warn ("[WRN] interface %s: ospf_check_md5 no key %d",
  ...
  if (nbr && ntohl(nbr->crypt_seqnum) > ntohl(ospfh->u.crypt.crypt_seqnum))
    {
      zlog_warn ("[WRN] interface %s: ospf_check_md5 bad sequence %d (expect %d)",
		 IF_NAME (oi),
  ...
  if (memcmp (pdigest, digest, OSPF_AUTH_MD5_SIZE))
    {
      zlog_warn ("[WRN] interface %s: ospf_check_md5 checksum mismatch",
		 IF_NAME (oi));
      return 0;
    }
...
ospf_make_md5_digest (struct ospf_interface *oi, struct ospf_packet *op)
{
  ...
  if (stream_get_endp(op->s) != op->length)
    /* XXX size_t */
    zlog_warn("[WRN] ospf_make_md5_digest: length mismatch stream %lu ospf_packet %u",
...
#define OSPF_WRITE_FRAG_SHIFT 3
  ...
  while ( (stream_get_endp(op->s) - stream_get_getp (op->s)) 
         > maxdatasize )
    {
      ...
      if (ret < 0)
        zlog_warn ("[WRN] ospf_write_frags: sendmsg failed to %s,"
      ...
      if (IS_DEBUG_OSPF_PACKET (type - 1, SEND))
        {
          zlog_debug ("[DBG] ospf_write_frags: sent id %d, off %d, len %d to %s\n",
                     iph->ip_id, iph->ip_off, iph->ip_len,
                     inet_ntoa (iph->ip_dst));
          if (IS_DEBUG_OSPF_PACKET (type - 1, DETAIL))
            {
              zlog_debug ("[DBG] -----------------IP Header Dump----------------------");
...
//  if (ospf->interface_type == ENNI)
//    zlog_debug("######## ospf_write, instance ENNI");
//  else
//    zlog_debug("######## ospf_write, instance INNI");
...
#ifdef GMPLS /* FIXME now it seams to be useless because of control plane */
//  if ((ospf->interface_type == ENNI) && 
//      (op->dst.s_addr == htonl (OSPF_ALLSPFROUTERS) || op->dst.s_addr == htonl (OSPF_ALLDROUTERS)))
//        ospf_if_ipmulticast (ospf, oi->address, oi->ifp->ifindex);
  ...
  //ret = scngw_sendmsg(ospf->fd, op->s->data, op->length, oi->address->u.prefix4, op->dst, cl_type);
  ret = scngw_sendmsg(ospf->fd, op->s->data, op->length, oi->address->u.prefix4, oi->connected->destination->u.prefix4, cl_type);

#else
  ret = scngwc_sendmsg (ospf->fd, op->s->data, op->length, oi->address->u.prefix4, op->dst, flags, &unread_pckts);
#endif /* GMPLS_NXW */
  if (unread_pckts)
	thread_add_event (master, ospf_read, ospf, 0);
#else
  ...
  if (ret < 0)
    zlog_warn ("[WRN] sendmsg in ospf_write failed to %s, "
  ...
  if (IS_DEBUG_OSPF_PACKET (type - 1, SEND))
      ...
      if (IS_DEBUG_OSPF_PACKET (type - 1, DETAIL))
	{
      ...
      zlog_debug ("[DBG] %s sent to [%s] via [%s].",
	    ...
	    struct stream * s, struct ospf_interface *oi, int size)
{
  ...
  if (IPV4_ADDR_SAME (&ospfh->router_id, &oi->ospf->router_id))
    {
      if (IS_DEBUG_OSPF_PACKET (ospfh->type - 1, RECV))
        {
          zlog_debug ("[DBG] ospf_header[%s/%s]: selforiginated, "
                     "dropping.",
  ...
  if (OSPF_IF_PASSIVE_STATUS (oi) == OSPF_IF_PASSIVE) {
    ...
    zlog_debug ("[DBG] ignoring HELLO from router %s sent to %s, "
  ...
  if (oi->type != OSPF_IFTYPE_POINTOPOINT 
    ...
    if (oi->address->prefixlen != p.prefixlen)
      {
	zlog_warn ("[WRN] Packet %s [Hello:RECV]: NetworkMask mismatch on %s (configured prefix length is %d, but hello packet indicates %d).",
  ...
  if (OSPF_IF_PARAM (oi, v_wait) != ntohl (hello->dead_interval))
    {
      zlog_warn ("[WRN] Packet %s [Hello:RECV]: RouterDeadInterval mismatch "
  ...
  if (OSPF_IF_PARAM (oi, fast_hello) == 0)
    {
      if (OSPF_IF_PARAM (oi, v_hello) != ntohs (hello->hello_interval))
        {
          zlog_warn ("[WRN] Packet %s [Hello:RECV]: HelloInterval mismatch "
		     "(expected %u, but received %u).",
		     inet_ntoa(ospfh->router_id),
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] Packet %s [Hello:RECV]: Options %s",
	       inet_ntoa (ospfh->router_id),
...
#define REJECT_IF_TBIT_ON	1 /* XXX */
  ...
  if (CHECK_FLAG (hello->options, OSPF_OPTION_T))
    {
       ...
       */
      zlog_warn ("[WRN] Packet %s [Hello:RECV]: T-bit on, drop it.",
		 inet_ntoa (ospfh->router_id));
      return;
    }
  ...
  if (CHECK_FLAG (oi->ospf->config, OSPF_OPAQUE_CAPABLE)
      && CHECK_FLAG (hello->options, OSPF_OPTION_O))
    {
       ...
       */
      zlog_warn ("[WRN] Packet %s [Hello:RECV]: O-bit abuse?",
		 inet_ntoa (ospfh->router_id));
  ...
  if (oi->area->external_routing == OSPF_AREA_NSSA) 
	...
	{
	  zlog_warn ("[WRN] NSSA-Packet-%s[Hello:RECV]: my options: %x, his options %x", inet_ntoa (ospfh->router_id), OPTIONS (oi), hello->options);
	  return;
	}
      if (IS_DEBUG_OSPF_NSSA)
        zlog_debug ("[DBG] NSSA-Hello:RECV:Packet from %s:", inet_ntoa(ospfh->router_id));
    }
  else    
	...
	CHECK_FLAG (hello->options, OSPF_OPTION_E))
      {
	zlog_warn ("[WRN] Packet %s [Hello:RECV]: my options: %x, his options %x",
...
#else
  OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_HelloReceived);
#endif /*HAVE_OMNIORB */
...
#else
      OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_OneWayReceived);
#endif /*HAVE_OMNIORB */
...
#else
      OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_TwoWayReceived);
#endif /*HAVE_OMNIORB */
		   ...
		   u_int16_t size)
{
  ...
  for (size -= OSPF_DB_DESC_MIN_SIZE;
	...
	{
	  zlog_warn ("[WRN] Packet [DD:RECV]: Unknown LS type %d.", lsah->type);
      ...
      &&  ! CHECK_FLAG (nbr->options, OSPF_OPTION_O))
        {
          zlog_warn ("[WRN] LSA[Type%d:%s]: Opaque capability mismatch?", lsah->type, inet_ntoa (lsah->id));
          OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
          return;
        }
#endif /* HAVE_OPAQUE_LSA */
	...
	case OSPF_OPAQUE_AS_LSA:
          ...
          if (oi->area->external_routing == OSPF_AREA_STUB)
            {
              zlog_warn ("[WRN] Packet [DD:RECV]: LSA[Type%d:%s] from %s area.",
      ...
      switch (ospf_lsa_more_recent (find, new))
        {
          ...
          default:
             ...
             */
            if (IS_DEBUG_OSPF_EVENT)
              zlog_debug ("[DBG] Packet [DD:RECV]: LSA received Type %d, "
	      ...
	      struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  if (nbr == NULL)
    {
      zlog_warn ("[WRN] Packet[DD]: Unknown Neighbor %s",
		 inet_ntoa (ospfh->router_id));
      return;
    }
  ...
  if ((OSPF_IF_PARAM (oi, mtu_ignore) == 0) && 
    ...
    {
      zlog_warn ("[WRN] Packet[DD]: Neighbor %s MTU %u is larger than [%s]'s MTU %u",
  ...
  if ( (oi->area->external_routing == OSPF_AREA_NSSA)
    ...
    {
      if (IS_DEBUG_OSPF_EVENT) 
        zlog_debug ("[DBG] Packet[DD]: Neighbour %s: Has NSSA capability, sends with N bit clear in DD options",
  ...
  if (CHECK_FLAG (dd->options, OSPF_OPTION_T))
    {
       ...
       */
      zlog_warn ("[WRN] Packet[DD]: Neighbor %s: T-bit on?", inet_ntoa (nbr->router_id));
      return;
    }
#endif /* REJECT_IF_TBIT_ON */
  ...
  switch (nbr->state)
    ...
    case NSM_Attempt:
    case NSM_TwoWay:
      zlog_warn ("[WRN] Packet[DD]: Neighbor %s state is %s, packet discarded.",
		 inet_ntoa(nbr->router_id),
  ...
  /* If the new state is ExStart, the processing of the current
	  ...
	  (size == OSPF_DB_DESC_MIN_SIZE))
	{
	  if (IPV4_ADDR_CMP (&nbr->router_id, &oi->ospf->router_id) > 0)
	    {
	      /* We're Slave---obey */
	      zlog_info ("[DBG] Packet[DD]: Neighbor %s Negotiation done (Slave).",
	      		 inet_ntoa(nbr->router_id));
	      nbr->dd_seqnum = ntohl (dd->dd_seqnum);
	  ...
	  else
	    {
	      ...
	      zlog_info ("[DBG] Packet[DD]: Neighbor %s: Initial DBD from Slave, "
	...
	{
	  zlog_info ("[DBG] Packet[DD]: Neighbor %s Negotiation done (Master).",
		     inet_ntoa(nbr->router_id));
          /* Reset I, leaving MS */
	...
	{
	  zlog_warn ("[WRN] Packet[DD]: Neighbor %s Negotiation fails.",
		     inet_ntoa(nbr->router_id));
	  break;
	}
      ...
      if (CHECK_FLAG (oi->ospf->config, OSPF_OPAQUE_CAPABLE))
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("[DBG] Neighbor[%s] is %sOpaque-capable.",
		       inet_ntoa (nbr->router_id),
          ...
          &&  IPV4_ADDR_SAME (&DR (oi), &nbr->address.u.prefix4))
            {
              zlog_warn ("[WRN] DR-neighbor[%s] is NOT opaque-capable; "
...
#else
      OSPF_NSM_EVENT_EXECUTE_NO_LOCK (nbr, NSM_NegotiationDone);
#endif
      ...
      if (ospf_db_desc_is_dup (dd, nbr))
	{
	  if (IS_SET_DD_MS (nbr->dd_flags))
	    ...
	    zlog_info ("[INF] Packet[DD] (Master): Neighbor %s packet duplicated.",
		       inet_ntoa (nbr->router_id));
	  else
	    /* Slave: cause to retransmit the last Database Description. */
	    {
	      zlog_info ("[INF] Packet[DD] [Slave]: Neighbor %s packet duplicated.",
			 inet_ntoa (nbr->router_id));
	...
	{
	  zlog_warn ("[WRN] Packet[DD]: Neighbor %s MS-bit mismatch.",
		     inet_ntoa(nbr->router_id));
	  OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] Packet[DD]: dd->flags=%d, nbr->dd_flags=%d",
	...
	{
	  zlog_info ("[INF] Packet[DD]: Neighbor %s I-bit set.",
		     inet_ntoa(nbr->router_id));
	  OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
	  break;
	}
      ...
      if (dd->options != nbr->options)
	{
	  ...
	  zlog_warn ("[WRN] Packet[DD]: Neighbor %s options mismatch.",
		     inet_ntoa(nbr->router_id));
	  OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
	  break;
	}
	...
	{
	  zlog_warn ("[WRN] Packet[DD]: Neighbor %s sequence number mismatch.",
		     inet_ntoa(nbr->router_id));
	  OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_SeqNumberMismatch);
	  break;
	}
      ...
      if (ospf_db_desc_is_dup (dd, nbr))
	{
	  if (IS_SET_DD_MS (nbr->dd_flags))
	    {
	      ...
	      zlog_info ("[INF] Packet[DD]: Neighbor %s duplicated, "
	                 "packet discarded.",
			inet_ntoa(nbr->router_id));
	      break;
	    }
      ...
      break;
    default:
      zlog_warn ("[WRN] Packet[DD]: Neighbor %s NSM illegal status %u.",
	     ...
	     struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  if (nbr == NULL)
    {
      zlog_warn ("[WRN] Link State Request: Unknown Neighbor %s.",
		 inet_ntoa (ospfh->router_id));
      return;
    }
  ...
  if (nbr->state != NSM_Exchange &&
      ...
      nbr->state != NSM_Full)
    {
      zlog_warn ("[WRN] Link State Request received from %s: "
                      ...
                      struct ospf_interface *oi, size_t size)
{
  ...
  for (; size >= OSPF_LSA_HEADER_SIZE && count > 0;
       size -= length, stream_forward_getp (s, length), count--)
    {
      ...
      if (length > size)
	{
	  zlog_warn ("[WRN] Link State Update: LSA length exceeds packet size.");
	  break;
	}
	...
	{
	  zlog_warn ("[WRN] Link State Update: LSA checksum error %x, %x.",
		     sum, lsah->checksum);
	  continue;
	}
      ...
      if (lsah->type < OSPF_MIN_LSA || lsah->type >= OSPF_MAX_LSA)
	{
	  zlog_warn ("[WRN] Link State Update: Unknown LS type %d", lsah->type);
	  continue;
	}
	  ...
	  ||  (! IS_OPAQUE_LSA(lsah->type) &&
               CHECK_FLAG (lsah->options, OSPF_OPTION_O)))
            {
               ...
               */
              zlog_warn ("[WRN] LSA[Type%d:%s]: O-bit abuse?", lsah->type, inet_ntoa (lsah->id));
              continue;
            }
#endif /* STRICT_OBIT_USAGE_CHECK */
	      ...
	      && nbr->oi->area->external_routing != OSPF_AREA_DEFAULT) 
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[WRN] LSA[Type%d:%s]: We are a stub, don't take this LSA.", lsah->type, inet_ntoa (lsah->id));
              continue;
            }
        }
      ...
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug("[DBG] LSA[Type%d:%s]: %p new LSA created with Link State Update",
...
/* Cleanup Update list. */
static void
ospf_upd_list_clean (struct zlist *lsas)
{
	     ...
	     struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  if (nbr == NULL)
    {
      zlog_warn ("[WRN] Link State Update: Unknown Neighbor %s on int: %s",
		 inet_ntoa (ospfh->router_id), IF_NAME (oi));
      return;
    }
  ...
  if (nbr->state < NSM_Exchange)
    {
      zlog_warn ("[WRN] Link State Update: "
...
#define DISCARD_LSA(L,N) {\
        if (IS_DEBUG_OSPF_EVENT) \
          zlog_debug ("[DBG] ospf_lsa_discard() in ospf_ls_upd() point %d: lsa %p Type-%d", N, lsa, (int) lsa->data->type); \
        ospf_lsa_discard (L); \
	continue; }
  ...
  for (ALL_LIST_ELEMENTS (lsas, node, nnode, lsa))
      ...
      if (IS_DEBUG_OSPF_NSSA)
	{
	  ...
	  zlog_debug("[DBG] LSA Type-%d from %s, ID: %s, ADV: %s",
		  lsa->data->type,
	 ...
	 translate them to a separate Type-5 packet.  */
        ...
        if (nbr->oi->area->external_routing != OSPF_AREA_DEFAULT) 
	  {
	    DISCARD_LSA (lsa, 1);
	    if (IS_DEBUG_OSPF_NSSA)
	      zlog_debug("[DBG] Incoming External LSA Discarded: We are NSSA/STUB Area");
	  }
	...
	if (nbr->oi->area->external_routing != OSPF_AREA_NSSA)
	  {
	    DISCARD_LSA (lsa,2);
	    if (IS_DEBUG_OSPF_NSSA)
	      zlog_debug("[DBG] Incoming NSSA LSA Discarded:  Not NSSA Area");
	  }
	   ...
	   ospf_nbr_count (oi, NSM_Loading)) == 0)
	{
	  ...
	  /* Discard LSA. */	  
	  zlog_info ("[INF] Link State Update[%s]: LS age is equal to MaxAge.",
		     dump_lsa_key(lsa));
          DISCARD_LSA (lsa, 3);
	}
      ...
      &&  IPV4_ADDR_SAME (&lsa->data->adv_router, &oi->ospf->router_id))
        {
          ...
          if (IS_LSA_MAXAGE (lsa))
            {
              zlog_warn ("[WRN] LSA[%s]: Boomerang effect?", dump_lsa_key (lsa));
              ospf_ls_ack_send (nbr, lsa);
              ospf_lsa_discard (lsa);
          ...
          if (current == NULL)
            {
              if (IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[DBG] LSA[%s]: Previously originated Opaque-LSA,"
                            "not found in the LSDB.", dump_lsa_key (lsa));

              SET_FLAG (lsa->flags, OSPF_LSA_SELF);
              
              ospf_opaque_self_originated_lsa_received (nbr, lsa);
              /*FIXME Adam. I Had to comment this line because in case of wrong LRM configuration ospf crashed (bad cc des address)
              The Lsa was twice disposed. Once in function ospf_opaque_self_originated_lsa_received and next after ospf_ls_ack_send. */
              /* ospf_ls_ack_send (nbr, lsa); */
              continue;
            }
        }
      ...
      {
        struct zlistnode *oinode, *oinnode;
        ...
        for (ALL_LIST_ELEMENTS (oi->ospf->oiflist, oinode, oinnode, out_if))
        {
              ...
              (!(IPV4_ADDR_SAME(&oi->ospf->router_id, &lsa->data->adv_router))))
          {
            if(out_if->network_lsa_self)
            {
              ...
              if(IS_DEBUG_OSPF_EVENT)
                zlog_debug ("[DBG] ospf_lsa_discard() in ospf_ls_upd() point 9: lsa %p Type-%d",
	...
	{
	  OSPF_NSM_EVENT_SCHEDULE (nbr, NSM_BadLSReq);
	  zlog_warn("[WRN] LSA[%s] instance exists on Link state request list",
	  	    dump_lsa_key(lsa));
	     ...
	     struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
  ...
  if (nbr == NULL)
    {
      zlog_warn ("[WRN] Link State Acknowledgment: Unknown Neighbor %s.",
		 inet_ntoa (ospfh->router_id));
      return;
    }
  ...
  if (nbr->state < NSM_Exchange)
    {
      zlog_warn ("[WRN] Link State Acknowledgment: "
      		 "Neighbor[%s] state %s is less than Exchange",
		 inet_ntoa (ospfh->router_id),
		 LOOKUP(ospf_nsm_state_msg, nbr->state));
      return;
...
ospf_recv_packet_iph (int fd, struct interface **ifp, struct stream *ibuf, struct ip **iph)
{
  int ret = 0;
  int16_t ip_len;
#ifdef GMPLS_NXW
  ret = scngw_recvmsg(ibuf->data, fd, iph, OSPF_MAX_PACKET_SIZE+1);
#else
  ret = scngwc_stream_recvmsg (ibuf->data, fd, iph, OSPF_MAX_PACKET_SIZE+1);
#endif /* GMPLS_NXW  */
  ibuf->endp += ret;
  if (ret < 0)
    {
      zlog_warn("[WRN] scngwc_stream_recvmsg failed");
      return NULL;
    }
  ...
  *ifp = NULL;
  ...
  if (ret != (ip_len - (int)sizeof (struct ip)))
    {
      zlog_warn ("[WRN] ospf_recv_packet_iph read length mismatch: SDU length is %d, "
       		 "but recvmsg returned %d", (ip_len - sizeof (struct ip)), ret);
      return NULL;
    }
...
ospf_recv_packet (int fd, struct interface **ifp, struct stream *ibuf)
{
  ...
  if (ret < 0)
    {
      zlog_warn("[WRN] stream_recvmsg failed: %s", safe_strerror(errno));
      return NULL;
    }
  if ((unsigned int)ret < sizeof(iph)) /* ret must be > 0 now */
    {
      zlog_warn("[WRN] ospf_recv_packet: discarding runt packet of length %d "
		"(ip header size is %u)",
  ...
  *ifp = if_lookup_by_index (ifindex);
  ...
  if (ret != ip_len)
    {
      zlog_warn ("[WRN] ospf_recv_packet read length mismatch: ip_len is %d, "
			  ...
			  struct ip *iph, struct ospf_header *ospfh)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->vlinks, node, vl_data))
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] associating packet with %s",
		       IF_NAME (vl_data->vl_oi));
	  if (! CHECK_FLAG (vl_data->vl_oi->ifp->flags, IFF_UP))
	    {
	      if (IS_DEBUG_OSPF_EVENT)
		zlog_debug ("[DBG] This VL is not up yet, sorry");
	      return NULL;
	    }
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] couldn't find any VL to associate the packet with");
...
ospf_check_sum (struct ospf_header *ospfh)
{
  ...
  if (ret != sum)
    {
      zlog_info ("[INF] ospf_check_sum(): checksum mismatch, my %X, his %X",
		 ret, sum);
      return 0;
    }
		    ...
		    struct ip *iph, struct ospf_header *ospfh)
{
  ...
  if (ospfh->version != OSPF_VERSION)
    {
      zlog_warn ("[WRN] interface %s: ospf_read version number mismatch.",
		 IF_NAME (oi));
      return -1;
    }
  ...
  if (!ospf_check_area_id (oi, ospfh))
    {
      zlog_warn ("[WRN] interface %s: ospf_read invalid Area ID %s.",
  ...
  if (! ospf_check_network_mask (oi, iph->ip_src))
    {
      zlog_warn ("[WRN] interface %s: ospf_read network address is not same [%s]",
  ...
  if (ospf_auth_type (oi) != ntohs (ospfh->auth_type))
    {
      zlog_warn ("[WRN] interface %s: auth-type mismatch, local %d, rcvd %d",
  ...
  if (! ospf_check_auth (oi, ibuf, ospfh))
    {
      zlog_warn ("[WRN] interface %s: ospf_read authentication failed.",
		 IF_NAME (oi));
      return -1;
    }
  ...
  if (ntohs (ospfh->auth_type) != OSPF_AUTH_CRYPTOGRAPHIC)
	...
	{
	  zlog_warn ("[WRN] interface %s: ospf_read packet checksum error %s",
	...
	{
	  zlog_warn ("[WRN] interface %s: ospf_read md5 authentication failed.",
		     IF_NAME (oi));
	  return -1;
	}
    }
...
ospf_read (struct thread *thread)
{
...
#ifdef GMPLS
  if (!(ibuf = ospf_recv_packet_iph (ospf->fd, &ifp, ospf->ibuf, &iph)))
    return -1;
#else
  ...
  if (ospf_if_lookup_by_local_addr (ospf, NULL, iph->ip_src))
    {
      if (IS_DEBUG_OSPF_PACKET (0, RECV))
        {
          zlog_debug ("[DBG] ospf_read[%s]: Dropping self-originated packet",
                     inet_ntoa (iph->ip_src));
        }
      return 0;
    }
  ...
  if ( (oi == NULL) ||
     ...
     )
    {
      if ((oi = ospf_associate_packet_vl (ospf, ifp, iph, ospfh)) == NULL)
        {
          if (IS_DEBUG_OSPF_EVENT)
            zlog_debug ("[DBG] Packet from [%s] received on link %s"
   ...
   */
    ...
    {
      zlog_warn ("[WRN] Packet from [%s] received on wrong link %s",
                 inet_ntoa (iph->ip_src), ifp->name); 
      return 0;
    }
  else if (oi->state == ISM_Down)
    {
      ...
      zlog_warn ("[WRN] Ignoring packet from %s to %s received on interface that is "
  ...
  && (oi->state != ISM_DR && oi->state != ISM_Backup))
    {
      zlog_warn ("[WRN] Dropping packet for AllDRouters from [%s] via [%s] (ISM: %s)",
  ...
  if (IS_DEBUG_OSPF_PACKET (ospfh->type - 1, RECV))
    {
      ...
      zlog_debug ("[DBG] %s received from [%s] via [%s]",
                 ospf_packet_type_str[ospfh->type],
                 inet_ntoa (ospfh->router_id), IF_NAME (oi));
      zlog_debug ("[DBG] src [%s],", inet_ntoa (iph->ip_src));
      zlog_debug ("[DBG] dst [%s]", inet_ntoa (iph->ip_dst));
  ...
  if (ret < 0)
    {
      if (IS_DEBUG_OSPF_PACKET (ospfh->type - 1, RECV))
        {
          zlog_debug ("[DBG] ospf_read[%s/%s]: Header check failed, "
                     "dropping.",
                     ospf_packet_type_str[ospfh->type],
                     inet_ntoa (iph->ip_src));
        }
...
ospf_make_hello (struct ospf_interface *oi, struct stream *s)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] make_hello: options: %x, int: %s",
...
static int
ospf_make_ls_upd (struct ospf_interface *oi, struct zlist *update, struct stream *s)
{
  struct ospf_lsa *lsa;
  struct zlistnode *node;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_make_ls_upd: Start");
  ...
  while ((node = listhead (update)) != NULL)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_make_ls_upd: List Iteration");
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_make_ls_upd: Stop");
  return length;
}

static int
ospf_make_ls_ack (struct ospf_interface *oi, struct zlist *ack, struct stream *s)
{
  struct zlistnode *node, *nnode;
...
{
#ifdef GMPLS
  if ((oi->ifp->adj_type == ENNI) && (oi->ifp->ospf_instance == INNI))
    return;
#endif /* GMPLS */
...
#ifdef GMPLS
  op = ospf_packet_new (oi->ifp->mtu - SCNGW_HDR_SIZE);
#else
...
ospf_ls_upd_packet_new (struct zlist *update, struct ospf_interface *oi)
{
  struct ospf_lsa *lsa;
  struct zlistnode *ln;
  size_t size;
  ...
  if ((OSPF_LS_UPD_MIN_SIZE + ntohs (lsa->data->length))
      > ospf_packet_max (oi))
    {
      if (!warned)
        {
          zlog_warn ("[WRN] ospf_ls_upd_packet_new: oversized LSA encountered!"
      ...
      if (IS_DEBUG_OSPF_PACKET (0, SEND))
        zlog_debug ("[DBG] ospf_ls_upd_packet_new: oversized LSA id:%s,"
       ...
       */
#ifdef GMPLS
      size = ntohs (lsa->data->length) + (oi->ifp->mtu - SCNGW_HDR_SIZE - ospf_packet_max (oi))
    ...
    {
#ifdef GMPLS
    size = oi->ifp->mtu - SCNGW_HDR_SIZE;
#else
    size = oi->ifp->mtu;
#endif /* GMPLS */
    }
  ...
  if (size > OSPF_MAX_PACKET_SIZE)
    {
      zlog_warn ("[WRN] ospf_ls_upd_packet_new: oversized LSA id:%s too big,"
...
static void
ospf_ls_upd_queue_send (struct ospf_interface *oi, struct zlist *update,
			struct in_addr addr)
{
  struct ospf_packet *op;
  u_int16_t length = OSPF_HEADER_SIZE;
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] listcount = %d, dst %s", listcount (update), inet_ntoa(addr));
...
ospf_ls_upd_send_queue_event (struct thread *thread)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ls_upd_send_queue start");
  ...
  if (again != 0)
    {
      if (IS_DEBUG_OSPF_EVENT)
        zlog_debug ("[DBG] ospf_ls_upd_send_queue: update lists not cleared,"
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_ls_upd_send_queue stop");
  
  return 0;
}

void
ospf_ls_upd_send (struct ospf_neighbor *nbr, struct zlist *update, int flag)
{
  struct ospf_interface *oi;
  struct ospf_lsa *lsa;
  ...
  if (oi->type == OSPF_IFTYPE_NBMA)
	...
	zlog_warn ("[WRN] LS-Update is directly sent on NBMA network.");
      if (IPV4_ADDR_SAME(&oi->address->u.prefix4, &p.prefix.s_addr))
	zlog_warn ("[WRN] LS-Update is sent to myself.");
    }
...
static void
ospf_ls_ack_send_list (struct ospf_interface *oi, struct zlist *ack,
		       struct in_addr dst)
{
  struct ospf_packet *op;
  u_int16_t length = OSPF_HEADER_SIZE;
