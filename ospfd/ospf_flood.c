...
ospf_flood_delayed_lsa_ack (struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
  ...
  listnode_add (inbr->oi->ls_ack, ospf_lsa_lock (lsa)); /* delayed LSA Ack 192.168.90.146
       - adj type             : ENNI */
}
				  ...
				  struct ospf_lsa *new, struct ospf_area *area)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] LSA[Type%d:%s]: Process self-originated LSA seq 0x%x",
	    ...
	    struct ospf_lsa *current, struct ospf_lsa *new)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] LSA[Flooding]: start, NBR %s (%s), cur(%p), New-LSA[%s]",
  ...
  if (current != NULL)		/* -- endo. */
    {
      ...
      &&  ntohl (current->data->ls_seqnum) == OSPF_INITIAL_SEQUENCE_NUMBER))
        {
          if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] LSA[Flooding]: Got a self-originated LSA, "
        ...
        {
          if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] LSA[Flooding]: LSA is received recently.");
          return -1;
        }
    }
			      ...
			      struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_flood_through_interface(): "
  ...
  for (rn = route_top (oi->nbrs); rn; rn = route_next (rn))
	...
	continue;
      ...
      onbr = rn->info;
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_flood_through_interface(): considering nbr %s (%s)",
	...
	{
	  if (IS_DEBUG_OSPF_EVENT)
	    zlog_debug ("[DBG] ospf_flood_through_interface(): nbr adj is not Full");
          ...
          if (! CHECK_FLAG (onbr->options, OSPF_OPTION_O))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug ("[DBG ]Skip this neighbor: Not Opaque-capable.");
              continue;
            }
          ...
          &&  onbr->state == NSM_Full)
            {
              ...
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug ("[DBG] Skip this neighbor: Initial flushing done.");
              continue;
            }
        }
#endif /* HAVE_OPAQUE_LSA */
      ...
      if (inbr)
        {
          ...
          if (IPV4_ADDR_SAME (&inbr->router_id, &onbr->router_id))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug ("[DBG] Skip this neighbor: inbr == onbr");
              continue;
            }
        }
      else
        {
          ...
          if (IPV4_ADDR_SAME (&lsa->data->adv_router, &onbr->router_id))
            {
              if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
                zlog_debug ("[DBG] Skip this neighbor: lsah->adv_router == onbr");
              continue;
            }
        }
      ...
      if ((lsa->data->type == OSPF_OPAQUE_LINK_LSA) && (lsa->oi != oi))
        {
           ...
           */
          if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
            zlog_debug ("[DBG] Type-9 Opaque-LSA: lsa->oi(%p) != oi(%p)", lsa->oi, oi);
          continue;
        }
#endif /* HAVE_OPAQUE_LSA */
		       ...
		       struct ospf_lsa *lsa)
{
  ...
  if (CHECK_FLAG (lsa->flags, OSPF_LSA_LOCAL_XLT)) /* Translated from 7  */
    if (IS_DEBUG_OSPF_NSSA)
      zlog_debug ("[DBG] Flood/AS: NSSA TRANSLATED LSA");

  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
    {
      int continue_flag = 0;
      struct zlistnode *if_node;
      struct ospf_interface *oi;
		    ...
		    struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
  ...
  switch (lsa->data->type)
    ...
    case OSPF_AS_NSSA_LSA:
      ...
      if (IS_DEBUG_OSPF_NSSA)
	zlog_debug ("[DBG] ospf_flood_through: LOCAL NSSA FLOOD of Type-7.");
      break;
    default:
      break;
    }
...
ospf_ls_request_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
   ...
   */
  if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
      zlog_debug ("[DBG] RqstL(%lu)++, NBR(%s), LSA[%s]",
...
ospf_ls_retransmit_add (struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
  ...
  if (ospf_lsa_more_recent (old, lsa) < 0)
      ...
      if (old)
	{
       ...
       */
      if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
	  zlog_debug ("[DBG] RXmtL(%lu)++, NBR(%s), LSA[%s]",
				    ...
				    struct ospf_lsa *lsa)
{
  ...
  {
    if (IS_DEBUG_OSPF_EVENT)
      zlog_debug("[DBG] ospf_ls_retransmit_delete_nbr_area: interface %s ar %s", oi->ifp->name, inet_ntoa(lsa->data->adv_router));
