...
ospf_check_abr_status (struct ospf *ospf)
{
  ...
  if (IS_DEBUG_OSPF_EVENT)
    zlog_debug ("[DBG] ospf_check_abr_status(): Start");
  ...
  if (IS_DEBUG_OSPF_EVENT)
    {
      zlog_debug ("[DBG] ospf_check_abr_status(): looked through areas");
      zlog_debug ("[DBG] ospf_check_abr_status(): bb_configured: %d", bb_configured);
      zlog_debug ("[DBG] ospf_check_abr_status(): bb_act_attached: %d",
		 bb_act_attached);
      zlog_debug ("[DBG] ospf_check_abr_status(): areas_configured: %d",
		 areas_configured);
      zlog_debug ("[DBG] ospf_check_abr_status(): areas_act_attached: %d",
		 areas_act_attached);
    }
  ...
  if (new_flags != ospf->flags)
    {
      ...
      if (IS_DEBUG_OSPF_EVENT)
	zlog_debug ("[DBG] ospf_check_abr_status(): new router flags: %x",new_flags);
  ...
  if (old)
    {
      ...
      if (GET_METRIC (sl->metric) == cost)
      ...
      else
        {
	      ...
	      zlog_warn ("[WRN] %s: Could not refresh %s to %s",
	                 __func__,
	                 buf,
	                 inet_ntoa (area->area_id));
	      return;
      ...
      if (!lsa)
      	{
      	  ...
      	  prefix2str ((struct prefix *)p, buf, sizeof(buf));
	  zlog_warn ("[WRN] %s: Could not originate %s to %s",
	             __func__,
	             buf,
		     inet_ntoa (area->area_id));
	  return;
      ...
      if (!lsa)
        {
          ...
          prefix2str ((struct prefix *)p, buf, sizeof(buf));
          zlog_warn ("[WRN] %s: Could not refresh/originate %s to %s",
                     __func__,
                     buf,
                     inet_ntoa (area->area_id));
          return;
