...
#else
/*       "router (ospf|ospf-inni|ospf-enni|ospf-uni)", */
       "router (ospf|ospf-inni|ospf-enni|ospf-uni-network|ospf-uni-client)",
       "Enable a routing process\n"
       "Start OSPF INNI configuration\n"
       "Start OSPF INNI configuration\n"
       "Start OSPF ENNI configuration\n"
       "Start OSPF UNI NETWORK configuration\n"
       "Start OSPF UNI CLIENT configuration\n"
/*       "Start OSPF UNI configuration\n" */
#endif /* GMPLS */
	)
{
  vty->node = OSPF_NODE;
  ...
  vty->index =  ospf_get ();
#else
  if ((strcmp(argv[0], "ospf") == 0) || (strcmp(argv[0], "ospf-inni") == 0))
    vty->index =  ospf_inni_get();

  else if (strcmp(argv[0], "ospf-enni") == 0)
    vty->index =  ospf_enni_get();
  else if (strcmp(argv[0], "ospf-uni-network") == 0)
  {
    vty->index =  ospf_uni_get();
    struct ospf* ospf = (struct ospf*) vty->index;
    ospf->interface_side = NETWORK;
  }
  else if (strcmp(argv[0], "ospf-uni-client") == 0)
  {
    vty->index =  ospf_uni_get();
    struct ospf* ospf = (struct ospf *) vty->index;
    ospf->interface_side = CLIENT;
  }
...
DEFUN (no_router_ospf,
       ...
       "no router ospf",
#else
       "no router (ospf|ospf-inni|ospf-enni|ospf-uni)",
#endif /* GMLS */
       NO_STR
       "Enable a routing process\n"
#ifndef GMPLS
                   ...
                   )
{
  ...
  ospf = ospf_lookup ();
#else
  ospf = (strcmp(argv[0], "ospf-enni") == 0) ? ospf_enni_lookup () : (strcmp(argv[0], "ospf-uni") == 0) ? ospf_uni_lookup () : ospf_inni_lookup ();
#endif /* GMPLS */
...
#else
      vty_out (vty, "There isn't active ospf %s instance %s", (strcmp(argv[0], "ospf-enni") == 0) ? "ENNI" : (strcmp(argv[0], "ospf-uni") == 0) ? "UNI" : "INNI", VTY_NEWLINE);
#endif /* GMPLS */
    return CMD_WARNING;
  }
       ...
       "OSPF router-id in IP address format\n")
{
  ...
  if (ospf->instance == INNI) {
	  /* this is a I-NNI instance and router-id is fetched from LRM */
	  vty_out (vty, 
		   "Router ID %s for INNI instance cannot be "
		   "set via OSPF VTY: it's fetched from LRM%s", 
		   argv[0], VTY_NEWLINE);
	  return CMD_WARNING;
  }
#endif /* GMPLS */
...
#ifdef GMPLS
DEFUN (ospf_router_rc,
       ospf_router_rc_cmd,
       "ospf rc if-addr A.B.C.D ADDR_LEN",
       "OSPF specific commands\n"
       "Enable routing controller\n"
       "Configure Router Controller interface address\n"
       "Router Controller if IP address format\n"
       "Router Controller if address length\n")
{
  struct ospf *ospf = vty->index;
  ...
  if (ospf->instance != ENNI)
  {
    vty_out (vty, "Router RC %s may be enabled only on ospf ENNI instance%s", argv[0], VTY_NEWLINE);
    return CMD_WARNING;
  }

  int addr_len = strtoul (argv[1], NULL, 10);
  createRC(rc_id, addr_len);
...
DEFUN (show_ip_ospf,
       ...
       "show ip ospf",
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni)",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
       "OSPF information\n"
#else
       "OSPF INNI information\n"
       "OSPF INNI information\n"
       "OSPF ENNI information\n"
       "OSPF UNI information\n"
#endif /* GMPLS */
       )
{
  struct ospf *ospf;
			    ...
			    struct interface *ifp)
{
...
#ifdef GMPLS
  /** Cheching if interface ifp belong to ospf instance */
  if (ifp->adj_type != ospf->instance)
  {
    return;
  }
#endif /* GMPLS */
  /* Is interface up? */
  vty_out (vty, "%s is %s ospf_instance: %s%s", ifp->name,
	   ((is_up = if_is_operative(ifp)) ? "up" : "down"), SHOW_ADJTYPE(ifp->ospf_instance), VTY_NEWLINE);
  ...
  for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
	...
	continue;
      ...
      vty_out (vty, "  os->state = %d", oi->state);
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) interface [INTERFACE]",
#endif /*GMPLS*/
       SHOW_STR
       IP_STR
#ifndef GMPLS
           ...
           )
{
  ...
  if (ospf == NULL)
  {
    vty_out (vty, "There isn't active %s instance %s", argv[0], VTY_NEWLINE);
    return CMD_WARNING;
  }
#endif /* GMPLS */
...
#ifdef GMPLS
DEFUN (show_ospf_instances,
       show_ospf_instances_cmd,
       "show ip ospf instances",
       SHOW_STR
       "OSPF information\n"
       "OSPF instances\n"
        )
{
  ...
  {
    switch (ospf->instance)
    {
      case INNI:
        vty_out (vty, " OSPF INNI instance%s", VTY_NEWLINE);
        break;

      case ENNI:
        vty_out (vty, " OSPF ENNI instance%s", VTY_NEWLINE);
        break;

      case UNI:
        vty_out (vty, " OSPF UNI instance%s", VTY_NEWLINE);
        break;
    }
  }
  return CMD_SUCCESS;
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor IFNAME",
#endif /*GMPLS*/
       SHOW_STR
       IP_STR
#ifndef GMPLS
        ...
        )
{
  ...
  ifp = if_lookup_by_name (argv[0+CLI_ARG_GMPLS]);
  if (!ifp)
    {
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor A.B.C.D",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
       ...
       )
{
  ...
  ret = inet_aton (argv[0+CLI_ARG_GMPLS], &router_id);
  if (!ret)
    {
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor detail",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor detail all",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) neighbor IFNAME detail",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
static int
ospf_te_show_link_subtlv (struct vty *vty, struct te_tlv_header *tlvh0, u_int16_t total)
{
  struct te_tlv_header *tlvh, *next = NULL;
  u_int16_t sum = 0;
  ...
  for (tlvh = tlvh0; sum < total; tlvh = (next ? next : TLV_HDR_NEXT (tlvh)))
  {
    ...
    switch (ntohs (tlvh->type))
    {
      case TE_LINK_SUBTLV_LCLIF_IPADDR:
        lcllif = (struct te_link_subtlv_lclif_ipaddr *) tlvh;
        vty_out(vty, "%s",  inet_ntoa (lcllif->value[0]));
        break;
      case TE_LINK_SUBTLV_LINK_LCL_RMT_IDS:
        lclid = (struct te_link_subtlv_link_lcl_rmt_ids *) tlvh;
        tmp.s_addr = lclid->local_id;
        vty_out(vty, "%s", inet_ntoa(tmp));
        break;
    }
    sum +=TLV_SIZE (tlvh);
  }
...
static int
ospf_tna_show_link_subtlv  (struct vty *vty, struct te_tlv_header *tlvh0, u_int16_t total)
{
  ...
  for (tlvh = tlvh0; sum < total; tlvh = (next ? next : TLV_HDR_NEXT (tlvh)))
  {
    ...
    switch (ntohs (tlvh->type))
    {
      case TE_TNA_ADDR_SUBTLV_TNA_ADDR_IPV4:
        tna_ipv4 = (struct te_tna_addr_subtlv_tna_addr_ipv4 *) tlvh;
        if (vty != NULL)
          vty_out(vty, "%s",  inet_ntoa(tna_ipv4->value));
        else
          zlog_debug("%s",  inet_ntoa(tna_ipv4->value));
        break;
...
static int
show_te_lsa_summary (struct vty *vty, struct ospf_lsa *lsa)
{
  ...
  switch (ntohs (tlvh->type))
  {
    case TE_TLV_ROUTER_ADDR:
      ...
      vty_out (vty, " RouterAddr:%s", inet_ntoa(top_routerLSA->router_addr.value));
      break;
    ...
    case TE_TLV_LINK:
      ...
      vty_out(vty, " TE-link:");
      ospf_te_show_link_subtlv (vty, tlvh+1, l);
      break;
    case TE_TLV_TNA_ADDR:
      top = (struct te_tlv_link *)tlvh;
      l = ntohs (top->header.length);
      vty_out (vty, " TNA:");
      ospf_tna_show_link_subtlv(vty, tlvh+1, l);
      break;
    default:
      break;
  };
  ...
  if (lsa != NULL)
	...
	switch (lsa->data->type)
	  ...
	  case OSPF_OPAQUE_AREA_LSA:
           opaque_type = GET_OPAQUE_TYPE(ntohl (lsa->data->id.s_addr));
           switch (opaque_type)
           {
            case OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA:
                show_te_lsa_summary(vty, lsa);
                break;
           }
...
#ifdef HAVE_OPAQUE_LSA
  " --- type-8 ---",
  "Opaque-Type/Id  ADV Router      Age  Seq#       CkSum  TLV Info",
  "Opaque-Type/Id  ADV Router      Age  Seq#       CkSum  TLV Info",
  "Opaque-Type/Id  ADV Router      Age  Seq#       CkSum  TLV Info",
#endif /* HAVE_OPAQUE_LSA */
};
...
static void
show_lsa_detail_tna_proc (struct vty *vty, struct route_table *rt,
		      struct in_addr *id, struct in_addr *adv_router)
{
  struct prefix_ls lp;
  struct route_node *rn, *start;
  ...
  show_lsa_prefix_set (vty, &lp, NULL, adv_router);
  start = route_node_get (rt, (struct prefix *) &lp);
  if (start)
  {
    route_lock_node (start);
    for (rn = start; rn; rn = route_next_until (rn, start))
    {
      if ((lsa = rn->info))
      {
        ...
        {
          int do_show = 0;
          struct te_tlv_header *tlvh_tna = te_subtlv_lookup(lsa, TE_TLV_TNA_ADDR, TE_TNA_ADDR_SUBTLV_TNA_ADDR_IPV4);
          if (tlvh_tna != NULL)
          {
            struct te_tna_addr_subtlv_tna_addr_ipv4 *tlvh_tna_addr = (struct te_tna_addr_subtlv_tna_addr_ipv4 *) tlvh_tna;

            if (id == NULL)
              do_show = 1;
            else if (tlvh_tna_addr->value.s_addr == id->s_addr)
              do_show = 1;

            if (adv_router != NULL)
            {
              if (lsa->data->adv_router.s_addr != adv_router->s_addr)
                do_show = 0;
            }
            if (do_show == 1)
              show_function[lsa->data->type] (vty, lsa);
...
/**
 * Shows TE-link details
 * @param *vty
 * @param *route_table
 * @param *id
 * @param *adv_router
 * @param adjacency (0 intradomain, 1 interdomain, -1 don't care)
 */

static void
show_lsa_detail_te_link_proc (struct vty *vty, struct route_table *rt, struct in_addr *id, struct in_addr *adv_router, int adjacency)
{
  struct prefix_ls lp;
  struct route_node *rn, *start;
  struct ospf_lsa *lsa;
  ...
  if (start)
  {
    ...
    for (rn = start; rn; rn = route_next_until (rn, start))
    {
      if ((lsa = rn->info))
      {
        if (show_function[lsa->data->type] != NULL)
        {
          struct te_tlv_header *tlvh_te_link = te_subtlv_lookup(lsa, TE_TLV_LINK, TE_LINK_SUBTLV_LCLIF_IPADDR);
          if (tlvh_te_link != NULL)
          {
            struct te_link_subtlv_lclif_ipaddr *tlvh_te_link_id = (struct te_tna_addr_subtlv_tna_addr_ipv4 *) tlvh_te_link;

            if (id == NULL)
              show_function[lsa->data->type] (vty, lsa);
            else if (tlvh_te_link_id->value[0].s_addr == id->s_addr)
                show_function[lsa->data->type] (vty, lsa);
          }
        }
      }
...
static void
show_lsa_detail_ra_proc (struct vty *vty, struct route_table *rt,
		      struct in_addr *id, struct in_addr *adv_router)
{
  struct prefix_ls lp;
  struct route_node *rn, *start;
  ...
  if (start)
  {
    ...
    for (rn = start; rn; rn = route_next_until (rn, start))
    {
      if ((lsa = rn->info))
      {
        if (show_function[lsa->data->type] != NULL)
        {
          struct te_tlv_header *tlvh_ra = te_subtlv_lookup(lsa, TE_TLV_ROUTER_ADDR, TE_ROUTER_ADDR_SUBTLV_ROUTER_ADDR);
          if (tlvh_ra != NULL)
          {
            int do_show = 0;
            struct te_router_addr_subtlv_router_addr *tlvh_router_addr = (struct te_router_addr_subtlv_router_addr *) tlvh_ra;

            if (id == NULL)
              do_show = 1;
            else if (tlvh_router_addr->value.s_addr == id->s_addr)
              do_show = 1;
...
/** Show detail TNA opaque information
  * if id is NULL then show all TNAs.
  * @param vty
  * @param ospf
  * @param id
  * @param adv_router
  */
static void
show_lsa_detail_tna (struct vty *vty, struct ospf *ospf,
		 struct in_addr *id, struct in_addr *adv_router)
{
  struct zlistnode *node;
  struct ospf_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
  {
    vty_out (vty, "%s                (Area %s)%s%s", VTY_NEWLINE, ospf_area_desc_string (area), VTY_NEWLINE, VTY_NEWLINE);
    show_lsa_detail_tna_proc (vty, AREA_LSDB (area, OSPF_OPAQUE_AREA_LSA), id, adv_router);
  }
}
...
/** Show detail TNA opaque information
  ...
  */
static void
show_lsa_detail_intradomain_te_link (struct vty *vty, struct ospf *ospf,
		 struct in_addr *id, struct in_addr *adv_router)
{
  struct zlistnode *node;
  struct ospf_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
  {
    vty_out (vty, "%s                (Area %s)%s%s",
             VTY_NEWLINE,
             ospf_area_desc_string (area), VTY_NEWLINE, VTY_NEWLINE);
    show_lsa_detail_te_link_proc (vty, AREA_LSDB (area, OSPF_OPAQUE_AREA_LSA), id, adv_router, 0);
  }
}
...
/** Show detail TNA opaque information
  ...
  */
static void
show_lsa_detail_interdomain_te_link (struct vty *vty, struct ospf *ospf,
		 struct in_addr *id, struct in_addr *adv_router)
{
  struct zlistnode *node;
  struct ospf_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
  {
    vty_out (vty, "%s                (Area %s)%s%s",
             VTY_NEWLINE,
             ospf_area_desc_string (area), VTY_NEWLINE, VTY_NEWLINE);
    show_lsa_detail_te_link_proc (vty, AREA_LSDB (area, OSPF_OPAQUE_AREA_LSA), id, adv_router, 1);
  }
}
...
/** Show detail TNA opaque information
  ...
  */
static void
show_lsa_detail_router_area (struct vty *vty, struct ospf *ospf,
		 struct in_addr *id, struct in_addr *adv_router)
{
  struct zlistnode *node;
  struct ospf_area *area;
  ...
  for (ALL_LIST_ELEMENTS_RO (ospf->areas, node, area))
  {
    vty_out (vty, "%s                (Area %s)%s%s",
             VTY_NEWLINE,
             ospf_area_desc_string (area), VTY_NEWLINE, VTY_NEWLINE);
    show_lsa_detail_ra_proc (vty, AREA_LSDB (area, OSPF_OPAQUE_AREA_LSA), id, adv_router);
  }
}
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
        ...
        )
{
  ...
  if (strcmp (argv[0 + CLI_ARG_GMPLS], "router") == 0)
    type = OSPF_ROUTER_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "ne", 2) == 0)
    type = OSPF_NETWORK_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "ns", 2) == 0)
    type = OSPF_AS_NSSA_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "su", 2) == 0)
    type = OSPF_SUMMARY_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "a", 1) == 0)
    type = OSPF_ASBR_SUMMARY_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "ex", 2) == 0)
    type = OSPF_AS_EXTERNAL_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "se", 2) == 0)
  {
    show_ip_ospf_database_summary (vty, ospf, 1);
      return CMD_SUCCESS;
  }
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "m", 1) == 0)
  {
    show_ip_ospf_database_maxage (vty, ospf);
    return CMD_SUCCESS;
  }
#ifdef HAVE_OPAQUE_LSA
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "opaque-l", 8) == 0)
    type = OSPF_OPAQUE_LINK_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "opaque-ar", 9) == 0)
    type = OSPF_OPAQUE_AREA_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "opaque-as", 9) == 0)
    type = OSPF_OPAQUE_AS_LSA;
#endif /* HAVE_OPAQUE_LSA */
  else
  {
    if (strncmp (argv[0 + CLI_ARG_GMPLS], "tna", 3) == 0)
    {
      show_lsa_detail_tna (vty, ospf, NULL, NULL);
      return CMD_SUCCESS;
    }
    else if (strncmp (argv[0 + CLI_ARG_GMPLS], "in", 2) == 0) //inni-link
    {
      if (argc == 1 + CLI_ARG_GMPLS)
      {
        show_lsa_detail_intradomain_te_link (vty, ospf, NULL, NULL);
      }
      else
      {
        ret = inet_aton (argv[1 + CLI_ARG_GMPLS], &id);
        if (!ret)
          return CMD_WARNING;
        show_lsa_detail_intradomain_te_link (vty, ospf, &id, NULL);
      }
      return CMD_SUCCESS;
    }

    else if (strncmp (argv[0 + CLI_ARG_GMPLS], "en", 2) == 0) //enni-link
    {
      if (argc == 1 + CLI_ARG_GMPLS)
      {
        show_lsa_detail_interdomain_te_link (vty, ospf, NULL, NULL);
      }
      else
      {
        ret = inet_aton (argv[1 + CLI_ARG_GMPLS], &id);
        if (!ret)
          return CMD_WARNING;
        show_lsa_detail_interdomain_te_link (vty, ospf, &id, NULL);
      }
      return CMD_SUCCESS;
    }
    else if (strncmp (argv[0 + CLI_ARG_GMPLS], "router-a", 8) == 0)
    {
      show_lsa_detail_router_area (vty, ospf, NULL, NULL);
      return CMD_SUCCESS;
    }
    return CMD_WARNING;
  }
  ...
  else if (argc >= 2 + CLI_ARG_GMPLS)
  {
    ret = inet_aton (argv[1 + CLI_ARG_GMPLS], &id);
    if (!ret)
      return CMD_WARNING;
    /* `show ip ospf database LSA ID'. */
    if (argc == 2 + CLI_ARG_GMPLS)
      show_lsa_detail (vty, ospf, type, &id, NULL);
    /* `show ip ospf database LSA ID adv-router ADV_ROUTER'. */
    else if (argc == 3 + CLI_ARG_GMPLS)
    {
      if (strncmp (argv[2 + CLI_ARG_GMPLS], "s", 1) == 0)
        adv_router = ospf->router_id;
      else
      {
        ret = inet_aton (argv[2 + CLI_ARG_GMPLS], &adv_router);
          if (!ret)
            return CMD_WARNING;
      }
      vty_out(vty, "asked router id %s%s", inet_ntoa(adv_router), VTY_NEWLINE);
...
ALIAS (show_ip_ospf_database,
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR "|max-age|self-originate|tna|inni-link|enni-link|router-area)",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
       ...
       "TNAs\n"
       "Intradomain TE-links\n"
       "Interdomain TE-links\n"
       "Router Areas\n"
         )
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR "|inni-link|enni-link) A.B.C.D",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR "|inni-link|enni-link) A.B.C.D adv-router A.B.C.D",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR ") A.B.C.D (self-originate|)",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
DEFUN (show_ip_ospf_database_tna,
       show_ip_ospf_database_tna_cmd,
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database tna A.B.C.D",
       SHOW_STR
       IP_STR
       "Ospf INNI instance\n"
       "Ospf INNI instance\n"
        ...
        )
{
  ...
  if (!inet_aton (argv[1], &tna_addr))
    return CMD_WARNING;

  vty_out (vty, "%s       OSPF Router with ID (%s)%s%s", VTY_NEWLINE,
           inet_ntoa (ospf->router_id), VTY_NEWLINE, VTY_NEWLINE);

  show_lsa_detail_tna (vty, ospf, &tna_addr, NULL);

  return CMD_SUCCESS;
}

DEFUN (show_ip_ospf_database_ra,
       show_ip_ospf_database_ra_cmd,
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database router-area A.B.C.D",
       SHOW_STR
       IP_STR
       "Ospf INNI instance\n"
       "Ospf INNI instance\n"
        ...
        )
{
  ...
  if (!inet_aton (argv[1], &ra))
    return CMD_WARNING;

  vty_out (vty, "%s       OSPF Router with ID (%s)%s%s", VTY_NEWLINE,
           inet_ntoa (ospf->router_id), VTY_NEWLINE, VTY_NEWLINE);

  show_lsa_detail_router_area (vty, ospf, &ra, NULL);

  return CMD_SUCCESS;
}

DEFUN (show_ip_ospf_database_tna_ra_adv_router,
       show_ip_ospf_database_tna_ra_adv_router_cmd,
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (tna|router-area) adv-router A.B.C.D",
       SHOW_STR
       IP_STR
       "Ospf INNI instance\n"
       "Ospf INNI instance\n"
       ...
       "TNAs\n"
       "Router Areas\n"
       "Select advertaising router\n"
       "Advertaising router address\n"
        )
{
  struct ospf *ospf;
  int ret;
  ...
  if (strncmp(argv[1], "tna", 3) == 0)
    show_lsa_detail_tna (vty, ospf, NULL, &adv_router);
  else if (strncmp(argv[1], "router-a", 8) == 0)
    show_lsa_detail_router_area (vty, ospf, NULL, &adv_router);
  else
    return CMD_WARNING;
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR ") adv-router A.B.C.D",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
        ...
        )
{
  ...
  /* Set database type to show. */
  if (strncmp (argv[0 + CLI_ARG_GMPLS], "r", 1) == 0)
    type = OSPF_ROUTER_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "ne", 2) == 0)
    type = OSPF_NETWORK_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "ns", 2) == 0)
    type = OSPF_AS_NSSA_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "s", 1) == 0)
    type = OSPF_SUMMARY_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "a", 1) == 0)
    type = OSPF_ASBR_SUMMARY_LSA;
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "e", 1) == 0)
    type = OSPF_AS_EXTERNAL_LSA;
#ifdef HAVE_OPAQUE_LSA
  else if (strncmp (argv[0 + CLI_ARG_GMPLS], "opaque-l", 8) == 0)
    type = OSPF_OPAQUE_LINK_LSA;
  ...
  if (strncmp (argv[1 + CLI_ARG_GMPLS], "s", 1) == 0)
    adv_router = ospf->router_id;
  else
    {
      ret = inet_aton (argv[1 + CLI_ARG_GMPLS], &adv_router);
      if (!ret)
	return CMD_WARNING;
    }
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) database (" OSPF_LSA_TYPES_CMD_STR ") (self-originate|)",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
#else
       "ip (ospf|ospf-inni|ospf-enni|ospf-uni) authentication-key AUTH_KEY A.B.C.D",
#endif /* GMPLS */
       "IP Information\n"
#ifndef GMPLS
       "OSPF interface commands\n"
...
DEFUN (ip_ospf_hello_interval_global,
       ip_ospf_hello_interval_global_cmd,
       "ip ospf hello-interval-global <1-65535>",
       "IP Information\n"
       "OSPF interface commands\n"
       "Time between HELLO packets\n"
       "Seconds\n"
       "Address of interface")
{
  ...
  if(iflist->count != 0){
	  ...
	  conf_ospf_hello_interval = seconds;
...
#else
    OSPF_ISM_EVENT_EXECUTE_NO_LOCK (oi, ISM_InterfaceDown);
    OSPF_ISM_EVENT_EXECUTE_NO_LOCK (oi, ISM_InterfaceUp);
#endif /* HAVE_OMNIORB */
	}
    }
...
show_ip_ospf_route_router (struct vty *vty, struct route_table *rtrs)
{
  ...
  for (rn = route_top (rtrs); rn; rn = route_next (rn))
	...
	for (ALL_LIST_ELEMENTS_RO ((struct zlist *)rn->info, node, or))
          {
            if (flag++)
          vty_out (vty, "%24s", "");
...
DEFUN (show_ip_ospf_border_routers,
...
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) border-routers",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
       ...
       "Ospf UNI instance\n"
#endif /* GMPLS */
       "show all the ABR's and ASBR's for this area\n"
          )
{
  struct ospf *ospf;
...
DEFUN (show_ip_ospf_route,
       ...
       "show ip ospf route",
#else
       "show ip (ospf|ospf-inni|ospf-enni|ospf-uni) route",
#endif /* GMPLS */
       SHOW_STR
       IP_STR
#ifndef GMPLS
...
static int
ospf_config_instance_write (struct vty *vty, struct ospf *ospf)
{
  struct interface *ifp;
  struct ospf_interface *oi;
  struct zlistnode *node;
  ...
  /* `router ospf' print. */
  if (ospf->instance == UNI)
    vty_out (vty, "router ospf-uni interface %s%s", (ospf->interface_side == CLIENT) ? "client": "network", VTY_NEWLINE);
  else
    vty_out (vty, "router ospf%s%s", (ospf->instance == ENNI) ? "-enni": "-inni", VTY_NEWLINE);

  write++;

#ifdef HAVE_OSPF_TE
#ifdef GMPLS
  ospf_te_config_write_router1(vty, ospf->instance);
#endif /* GMPLS */
#endif /* HAVE_OSPF_TE */
  ...
  for (ALL_LIST_ELEMENTS_RO (om->iflist, node, ifp))
    if (OSPF_IF_PARAM_CONFIGURED (IF_DEF_PARAMS (ifp), passive_interface)
        && IF_DEF_PARAMS (ifp)->passive_interface != ospf->passive_interface_default)
    {
...
ospf_config_write (struct vty *vty)
{
  ...
  ospf = ospf_lookup ();
  if (ospf != NULL)
    write=ospf_config_instance_write(vty, ospf);
#else
...
ospf_vty_show_init (void)
{
  ...
  install_element (VIEW_NODE, &show_ip_ospf_database_tna_cmd);
  install_element (VIEW_NODE, &show_ip_ospf_database_ra_cmd);
  install_element (VIEW_NODE, &show_ip_ospf_database_tna_ra_adv_router_cmd);
  ...
  install_element (ENABLE_NODE, &show_ip_ospf_database_tna_cmd);
  install_element (ENABLE_NODE, &show_ip_ospf_database_ra_cmd);
  install_element (ENABLE_NODE, &show_ip_ospf_database_tna_ra_adv_router_cmd);
...
ospf_vty_if_init (void)
{
  ...
  install_element (CONFIG_NODE, &interface_global_cmd);
  ...
  install_element (INTERFACE_NODE, &ip_ospf_hello_interval_global_cmd);
...
#ifdef GMPLS
  install_element (ENABLE_NODE, &show_ospf_instances_cmd),
#endif /* GMPLS */
...
#ifdef GMPLS
  install_element (OSPF_NODE, &ospf_router_rc_cmd);
#endif /* GMPLS */
