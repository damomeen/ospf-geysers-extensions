			     ...
			     zebra_size_t length)
{
  ...
  uint32_t energyConsuption;

  adj_type_t adj = (adj_type_t)zebra_router_id_update_read(zclient->ibuf, &router_id, &energyConsuption);
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
  {
    char buf[128];
    prefix2str(&router_id, buf, sizeof(buf));
    zlog_debug("[DBG] Zebra rcvd: Router %s Id update to %s", SHOW_ADJTYPE(adj), buf);
  }
...
#else
  /* FIXME Adam: what about different instances ? zclient has no information about ospf instance */
  switch(adj)
  {
    case INNI:
  ...
  if (ospf != NULL)
  {
    ...
    /*FIXME TRYING TO FIX lrmd ROUTER-ID problem with interface states */
    ...
    /*FIXME end */
    router_id_update_te(adj, energyConsuption);        /* update information about local node id */

    /* read localnode id */
  }

#if 0
  /* layer 1 instance Router ID is RC ID 
   * and must not be changed from LRM/ZEBRA 
   */
  ospf = ospf_enni_lookup ();
  if (ospf != NULL)
    ospf_router_id_update (ospf);
...
ospf_interface_add (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
    zlog_debug ("[DBG] Zebra: Adding interface %s (index: %d, flags: %llu, metric: %d, mtu: %d, adj_type: %s, instance: %s)", ifp->name, ifp->ifindex, ifp->flags, ifp->metric, ifp->mtu, SHOW_ADJTYPE(ifp->adj_type), SHOW_ADJTYPE(ifp->ospf_instance));
  else
    zlog_debug ("[DBG] Zebra: Adding interface %s (index: %d, flags: %llu, metric: %d, mtu: %d, adj_type: %s, instance: %s)", ifp->name, ifp->ifindex, ifp->flags, ifp->metric, ifp->mtu, SHOW_ADJTYPE(ifp->adj_type), SHOW_ADJTYPE(ifp->ospf_instance));

    /*for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, ifc))
    {
	  zlog_debug("local ip: %s", inet_ntoa(ifc->address->u.prefix4));
	  zlog_debug("remote ip: %s", inet_ntoa(ifc->destination->u.prefix4));
  ...
  ospf = ospf_lookup ();
#else
  ospf = (ifp->ospf_instance == INNI) ? ospf_inni_lookup() : (ifp->ospf_instance == ENNI) ? ospf_enni_lookup() : ospf_uni_lookup() ;
#endif /* GMPLS */
  if (ospf != NULL)
    ospf_if_update (ospf);
  ...
  {
    zlog_debug("[DBG] Zebra: Setting special parameters for %s interface", ifp->name);
    ...
    for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
    {
      ...
      zlog_debug("[DBG] Setting oi -> NBMA");
#ifndef HAVE_OMNIORB
...
static
int ospf_interface_update(int command, struct zclient *zclient, zebra_size_t length){
	
	struct interface *ifp;
	struct ospf *ospf;

	zlog_debug("[DBG] ospf_interface_update");
	ifp = zebra_interface_update_read(zclient->ibuf);
  	if (!OSPF_IF_PARAM_CONFIGURED (IF_DEF_PARAMS (ifp), type))
    	{
      		SET_IF_PARAM (IF_DEF_PARAMS (ifp), type);
      		IF_DEF_PARAMS (ifp)->type = ospf_default_iftype(ifp);
                       ...
                       zebra_size_t length)
{
  ...
  if (if_is_up (ifp))
    zlog_warn ("[WRN] Zebra: got delete of %s, but interface is still up",
               ifp->name);
                            ...
                            zebra_size_t length)
{
  ...
  if (/*IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE)*/1)
    {
      //char buf[128];
      //char buf_dest[128];
      //prefix2str(c->destination, buf_dest, sizeof(buf));

      zlog_debug("[DBG] Zebra: Adding local interface address %s to %s", buf, c->ifp->name);
      //zlog_debug("TEL %s remote interface %s address added", c->ifp->name, buf_dest);
      
    }
#ifndef GMPLS
  ospf = ospf_get ();	/* ospf_get() avoids of ospfd crashing because of lack of ospf instance */
#else
  ospf = (c->ifp->ospf_instance == INNI) ? ospf_inni_get() : (c->ifp->ospf_instance == ENNI) ? ospf_enni_get() : ospf_uni_get();
#endif /* GMPLS */  
  /*create a prefix out of the local link id (ipv4 addresse) of 
  * the TE-link sent as an interface by lrmd
  */
  str2prefix_ipv4(buf, &p);   
  /*inet_aton("0.0.O.0", &area_id);*/
  /*set by default the area to 0.0.0.0: backbone ospf area*/
  bzero(&area_id, sizeof(struct in_addr)); 
  /*activate ospf on this network 
   * (this is equivalent to the line: network PREFIX area AREA_ID in ospfd.conf
   * */
              ...
              if (is_prefix_default (&p))
              ...
              else
                {
                  ...
                  if (!current)
                    ...
                    zlog_warn ("[WRN] ospf_zebra_read_ipv4() : %s already exists",
                               inet_ntoa (p.prefix));
                }
            }
        }
...
ospf_zebra_init ()
{
  ...
  ospf = ospf_inni_lookup ();
  if (ospf != NULL) {
    type_mask |= OSPF_INNI_CLIENT;
  }

  ospf = ospf_uni_lookup ();
  if (ospf != NULL) {
    type_mask |= OSPF_UNI_CLIENT;
  }

  ospf = ospf_enni_lookup ();
  if (ospf != NULL) {
    type_mask |= OSPF_ENNI_CLIENT;
  } 

  zclient_init (zclient, ZEBRA_ROUTE_OSPF, type_mask);
#else
  ...
  zclient->interface_te_update = ospf_interface_update;
