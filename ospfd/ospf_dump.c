...
ospf_packet_hello_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG] Hello");
  zlog_debug ("[DBG]   NetworkMask %s", inet_ntoa (hello->network_mask));
  zlog_debug ("[DBG]   HelloInterval %d", ntohs (hello->hello_interval));
  zlog_debug ("[DBG]   Options %d (%s)", hello->options,
	     ospf_options_dump (hello->options));
  zlog_debug ("[DBG]   RtrPriority %d", hello->priority);
  zlog_debug ("[DBG]   RtrDeadInterval %ld", (u_long)ntohl (hello->dead_interval));
  zlog_debug ("[DBG]   DRouter %s", inet_ntoa (hello->d_router));
  zlog_debug ("[DBG]   BDRouter %s", inet_ntoa (hello->bd_router));

  length -= OSPF_HEADER_SIZE + OSPF_HELLO_MIN_SIZE;
  zlog_debug ("[DBG]   # Neighbors %d", length / 4);
  for (i = 0; length > 0; i++, length -= sizeof (struct in_addr))
    zlog_debug ("[DBG]     Neighbor %s", inet_ntoa (hello->neighbors[i]));
}
...
ospf_lsa_header_dump (struct lsa_header *lsah)
{
  ...
  zlog_debug ("[DBG]   LSA Header");
  zlog_debug ("[DBG]     LS age %d", ntohs (lsah->ls_age));
  zlog_debug ("[DBG]     Options %d (%s)", lsah->options,
	     ospf_options_dump (lsah->options));
  zlog_debug ("[DBG]     LS type %d (%s)", lsah->type,
             (lsah->type ? lsah_type : "unknown type"));
  zlog_debug ("[DBG]     Link State ID %s", inet_ntoa (lsah->id));
  zlog_debug ("[DBG]     Advertising Router %s", inet_ntoa (lsah->adv_router));
  zlog_debug ("[DBG]     LS sequence number 0x%lx", (u_long)ntohl (lsah->ls_seqnum));
  zlog_debug ("[DBG]     LS checksum 0x%x", ntohs (lsah->checksum));
  zlog_debug ("[DBG]     length %d", ntohs (lsah->length));
}
...
ospf_router_lsa_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG]   Router-LSA");
  zlog_debug ("[DBG]     flags %s", 
	     ospf_router_lsa_flags_dump (rl->flags, buf, BUFSIZ));
  zlog_debug ("[DBG]     # links %d", ntohs (rl->links));

  len = ntohs (rl->header.length) - OSPF_LSA_HEADER_SIZE - 4;
  for (i = 0; len > 0; i++)
    {
      zlog_debug ("[DBG]     Link ID %s", inet_ntoa (rl->link[i].link_id));
      zlog_debug ("[DBG]     Link Data %s", inet_ntoa (rl->link[i].link_data));
      zlog_debug ("[DBG]     Type %d", (u_char) rl->link[i].type);
      zlog_debug ("[DBG]     TOS %d", (u_char) rl->link[i].tos);
      zlog_debug ("[DBG]     metric %d", ntohs (rl->link[i].metric));
...
ospf_network_lsa_dump (struct stream *s, u_int16_t length)
{
  ...
  */
  zlog_debug ("[DBG]     Network Mask %s", inet_ntoa (nl->mask));
  zlog_debug ("[DBG]     # Attached Routers %d", cnt);
  for (i = 0; i < cnt; i++)
    zlog_debug ("[DBG]       Attached Router %s", inet_ntoa (nl->routers[i]));
}
...
ospf_summary_lsa_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG]   Summary-LSA");
  zlog_debug ("[DBG]     Network Mask %s", inet_ntoa (sl->mask));

  size = ntohs (sl->header.length) - OSPF_LSA_HEADER_SIZE - 4;
  for (i = 0; size > 0; size -= 4, i++)
    zlog_debug ("[DBG]     TOS=%d metric %d", sl->tos,
...
ospf_as_external_lsa_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG]   %s", ospf_lsa_type_msg[al->header.type].str);
  zlog_debug ("[DBG]     Network Mask %s", inet_ntoa (al->mask));

  size = ntohs (al->header.length) - OSPF_LSA_HEADER_SIZE -4;
  for (i = 0; size > 0; size -= 12, i++)
    {
      zlog_debug ("[DBG]     bit %s TOS=%d metric %d",
		 IS_EXTERNAL_METRIC (al->e[i].tos) ? "E" : "-",
		 al->e[i].tos & 0x7f, GET_METRIC (al->e[i].metric));
      zlog_debug ("[DBG]     Forwarding address %s", inet_ntoa (al->e[i].fwd_addr));
      zlog_debug ("[DBG]     External Route Tag %d", al->e[i].route_tag);
    }
}
...
ospf_lsa_header_list_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG]   # LSA Headers %d", length / OSPF_LSA_HEADER_SIZE);
...
ospf_packet_db_desc_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG] Database Description");
  zlog_debug ("[DBG]   Interface MTU %d", ntohs (dd->mtu));
  zlog_debug ("[DBG]   Options %d (%s)", dd->options,
	     ospf_options_dump (dd->options));
  zlog_debug ("[DBG]   Flags %d (%s)", dd->flags,
	     ospf_dd_flags_dump (dd->flags, dd_flags, sizeof dd_flags));
  zlog_debug ("[DBG]   Sequence Number 0x%08lx", (u_long)ntohl (dd->dd_seqnum));
...
ospf_packet_ls_req_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG] Link State Request");
  zlog_debug ("[DBG]   # Requests %d", length / 12);
  ...
  for (; length > 0; length -= 12)
    {
      ...
      zlog_debug ("[DBG]   LS type %d", ls_type);
      zlog_debug ("[DBG]   Link State ID %s", inet_ntoa (ls_id));
      zlog_debug ("[DBG]   Advertising Router %s",
...
ospf_packet_ls_upd_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG] Link State Update");
  zlog_debug ("[DBG]   # LSAs %d", count);

  while (length > 0 && count > 0)
    {
      if (length < OSPF_HEADER_SIZE || length % 4 != 0)
	{
          zlog_debug ("[DBG]   Remaining %d bytes; Incorrect length.", length);
	  break;
	}
...
ospf_packet_ls_ack_dump (struct stream *s, u_int16_t length)
{
  ...
  zlog_debug ("[DBG] Link State Acknowledgment");
  ospf_lsa_header_list_dump (s, length);
...
{
  /* IP Header dump. */
  zlog_debug ("[DBG] ip_v %d", iph->ip_v);
  zlog_debug ("[DBG] ip_hl %d", iph->ip_hl);
  zlog_debug ("[DBG] ip_tos %d", iph->ip_tos);
  zlog_debug ("[DBG] ip_len %d", iph->ip_len);
  zlog_debug ("[DBG] ip_id %u", (u_int32_t) iph->ip_id);
  zlog_debug ("[DBG] ip_off %u", (u_int32_t) iph->ip_off);
  zlog_debug ("[DBG] ip_ttl %d", iph->ip_ttl);
  zlog_debug ("[DBG] ip_p %d", iph->ip_p);
  zlog_debug ("[DBG] ip_sum 0x%x", (u_int32_t) iph->ip_sum);
  zlog_debug ("[DBG] ip_src %s",  inet_ntoa (iph->ip_src));
  zlog_debug ("[DBG] ip_dst %s", inet_ntoa (iph->ip_dst));
}
...
ospf_header_dump (struct ospf_header *ospfh)
{
  ...
  zlog_debug ("[DBG] Header");
  zlog_debug ("[DBG]   Version %d", ospfh->version);
  zlog_debug ("[DBG]   Type %d (%s)", ospfh->type,
	     ospf_packet_type_str[ospfh->type]);
  zlog_debug ("[DBG]   Packet Len %d", ntohs (ospfh->length));
  zlog_debug ("[DBG]   Router ID %s", inet_ntoa (ospfh->router_id));
  zlog_debug ("[DBG]   Area ID %s", inet_ntoa (ospfh->area_id));
  zlog_debug ("[DBG]   Checksum 0x%x", ntohs (ospfh->checksum));
  zlog_debug ("[DBG]  AuType %d", ntohs (ospfh->auth_type));
  ...
  switch (ntohs (ospfh->auth_type))
    ...
    case OSPF_AUTH_SIMPLE:
      ...
      zlog_debug ("[DBG]   Simple Password %s", buf);
      break;
    case OSPF_AUTH_CRYPTOGRAPHIC:
      zlog_debug ("[DBG]   Cryptographic Authentication");
      zlog_debug ("[DBG]   Key ID %d", ospfh->u.crypt.key_id);
      zlog_debug ("[DBG]   Auth Data Len %d", ospfh->u.crypt.auth_data_len);
      zlog_debug ("[DBG]   Sequence number %ld",
		 (u_long)ntohl (ospfh->u.crypt.crypt_seqnum));
      break;
    default:
      zlog_debug ("[DBG] * This is not supported authentication type");
      break;
    }
