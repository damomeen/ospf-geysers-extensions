...
#include "lib/corba.h"
#include "ospf_corba.h"

#define USE_OSPF_CORBA 1
#define OLD_OSPF 0
...
main (int argc, char **argv)
{
  ...
  while (1) 
    {
      ...
      opt = getopt_long (argc, argv, "dlf:i:hA:P:u:g:avCo:", longopts, 0);
  ...
  if (!corba_init(iors_dir, CORBA_SERVANT_SCNGW_NETSERVICES, master)) {
    zlog_err("[ERR] OSPF-MAIN: Cannot initialize CORBA subsystem");
    exit(1);
  }
  if (!corba_scngw_client_setup()) {
    printf("[ERR] Cannot setup SCNGW client object\n");
    exit(1);
  }
#endif
#endif
  ...
  /* Print banner. */
  zlog_notice ("[DBG] OSPFd %s starting: vty@%d", QUAGGA_VERSION, vty_port);


#ifdef HAVE_OMNIORB
#ifdef USE_OSPF_CORBA

  zlog_debug("Initializing pcera corba client...");
  if (!corba_g2pcera_client_setup()) {
    zlog_err("[ERR] Cannot setup client object");
    //exit(1);
  }


  //if (!corba_gunigw_client_setup()) {
  //    zlog_err("[ERR] Cannot setup client object");
  //    exit(1);
  //}

#endif /* USE_OSPF_CORBA */
#endif /* HAVE_OMNIORB */
